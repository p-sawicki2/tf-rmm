...
- For pointer types:
  ...
  - Use ``void *`` is generally discouraged. Although it is useful to represent
    pointers to types that are abstracted away from the callers and has useful
    implicit cast properties, for the sake of a more uniform code base, we
    encourage use of ``uintptr_t`` where possible.
...
- Use ``uint64_t`` to store the contents of an AArch64 register
  or represent a 64 bit value. Use of ``unsigned long`` for these purposes is
  discouraged.
