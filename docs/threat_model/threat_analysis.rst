...
.. table:: Table 2: RMM Assets
  ...
  |                    | | Attestation Key) or see (e.g. secure logs or    |
  |                    |   debugging information such as crash reports) in |
  |                    | | or within RMM itself.                           |
  +--------------------+---------------------------------------------------+
  | Code Execution     | | This represents the requirement that Realms     |
  |                    |   should only run code in R-EL1/EL0 and that no   |
  |                    | | escalation of privilege or code injected by an  |
  |                    |   attacker should run at R-EL2.                   |
  |                    | | Also note that the NS Host should not be able   |
  |                    |   to inject code into the Realm world by a        |
  |                    | | mechanism other than the allowed RMM ABI.       |
  +--------------------+---------------------------------------------------+
  | Availability       | | Availability of Realm world is out of scope for |
  |                    |   this Threat Model. However, Realm and RMM       |
  |                    | | should not affect availability of NS Host nor   |
  |                    |   Secure World.                                   |
  +--------------------+---------------------------------------------------+
