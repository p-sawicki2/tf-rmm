...
-  When the ``RSI_ATTEST_TOKEN_CONTINUE`` call is interrupted and then resumed
   later by host by ``RMI_REC_ENTER``, then the original SMC is replayed again
   with the original arguments rather than ``RSI_INCOMPLETE`` error code being
   returned to realm. The result is that the interrupted RSI call is continued
