...
- Added capability to privately map the per-CPU stack.
  ...
  * This contains any stack overflows to the particluar CPU and prevents
...
- Added github workflow to configure an automatic message for PRs on GitHub
  and also build and run RMM unittests for every update  of `main` branch.
...
- Cache line align xlat lib data structures accessed by secondary CPUs to avoid
  data corruption due to mismatched memory attribute accesses by RMM duing warm
  boot.

- Corrected linker options when building qcbor library.

- Lot of fixes to comply to MISRA coding guidelines.
...
- Fixed issue with rsi_log_on_exit() and modifed the logging format.
...
- The compatibility checks for version in RMM-EL3 interface is hardened.
...
- Updated t_cose submodule to use upstream rather than a forked version.
...
- Some capabilities as mentioned in `RMM v1.0 EAC5 specification`_ are
  ...
  * The support for Self-hosted in Realms is not implemented.

  * Although the RMM allows attestation token sizes of larger than 4KB,
    there is a limitation in RMM-EL3 interface in that only a shared buffer
    of 4KB is currently shared on the FVP. This needs to be enhanced so that
    larger token sizes can be tested.
...
- Support for new capabilities like Device assignment as mandated by RMM
  specification.
