...
- Reduce the memory required for `struct granule`.
  ...
  *  The data structure is optimized such that it is 4 bytes is size
...
- Improvements to Cppcheck static analysis.
  ...
  *  Fixed MISRA violations detected by Cppcheck misra addon.
...
- Prototype new features as described in `RMM v1.1 Alpha specification`_.
  ...
  *  Realm Device Assignment - A feature which allows devives to be assigned to Realms,
     attested and granted permisiion to access Realm owned memory.
