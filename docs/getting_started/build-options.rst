...
Run the additional build target `coverage_report` after build and run of unit tests.
...
.. code-block:: bash
    ...
    cmake --build ${RMM_BUILD_DIR} -- run-unittests coverage-report

This above will automatically generate the HTML coverage report in folder `build/Testing/Debug/coverage` within build directory.
The HTML generation can be disabled by setting `RMM_HTML_COV_REPORT=OFF`.
...
.. csv-table:: RMM CMake Options Table
   ...
   RMM_HTML_COV_REPORT     , ON | OFF, ON, "Enable HTML output report for coverity analysis"
