...
1. **Sysreg and C runtime initialization phase.**
   ...
   - Any function in RMM which is invoked prior to MMU enable needs to perform
     ...
     invoked as part of cold boot or warm boot. In the warm boot path, care
     should be taken that only per-cpu locations are modified and reads of
     global data must be restricted to the locations which are required to be
