 ...
 * If enabled and endpoint is a Responder then
 * LIBSPDM_ENABLE_CAPABILITY_ENCAP_CAP must also be enabled.
 */
#define LIBSPDM_SEND_GET_CERTIFICATE_SUPPORT 0

/*
 * If 1 then endpoint supports sending CHALLENGE request. If enabled and
 * endpoint is a Responder then LIBSPDM_ENABLE_CAPABILITY_ENCAP_CAP must also be
 * enabled.
 */
