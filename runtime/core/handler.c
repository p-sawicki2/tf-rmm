...
typedef struct {
	unsigned long x[DUMP_REGS];
	unsigned long lr;
} dump_regs_t;

__dead2 static void fatal_abort(dump_regs_t *regs)
{
	__unused unsigned long sp_el2;
	__unused unsigned long sp_el0 =
		(unsigned long)regs + sizeof(dump_regs_t);
	__unused unsigned long lr = regs->lr;

	ERROR("Unexpected exception on CPU #%u:\n", my_cpuid());

	for (unsigned int i = 0U; i < DUMP_REGS; i += 2U) {
		ERROR("X%u:\t\t0x%016lx   X%u:\t\t0x%016lx\n",
	...
	ERROR("LR:\t\t0x%016lx\n", lr);
	ERROR("SP_EL0:\t\t0x%016lx\n", sp_el0);
	ERROR("SP_EL2:\t\t0x%016lx%c (0x%016lx-0x%016lx)\n", sp_el2,
	...
	ERROR("HCR_EL2:\t0x%016lx   E2H = %c\n", read_hcr_el2(),
		((read_hcr_el2() & HCR_E2H) == 0UL) ? '0' : '1');
	ERROR("CPTR_EL2:\t0x%016lx\n", read_cptr_el2());
	ERROR("MDCR_EL2:\t0x%016lx\n", read_mdcr_el2());
	ERROR("SCTLR_EL2:\t0x%016lx\n", read_sctlr_el2());
