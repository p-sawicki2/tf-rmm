...
#define RMI_STATUS_HANDLER(_id)[RMI_##_id] = #_id

const char *rmi_status_handler[] = {
	RMI_STATUS_HANDLER(SUCCESS),
	RMI_STATUS_HANDLER(ERROR_INPUT),
	RMI_STATUS_HANDLER(ERROR_REALM),
	RMI_STATUS_HANDLER(ERROR_REC),
	RMI_STATUS_HANDLER(ERROR_RTT),
	RMI_STATUS_HANDLER(ERROR_IN_USE)
};
COMPILER_ASSERT(ARRAY_LEN(rmi_status_handler) == RMI_ERROR_COUNT);
			    ...
			    struct smc_result *ret)
{
	...
	if ((handler->log_exec) ||
		...
		if (rc.status >= RMI_ERROR_COUNT) {
			...
			INFO(" > RMI_%s", rmi_status_handler[rc.status]);
		}
