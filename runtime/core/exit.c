...
{
	/*
	 * If this realm used SVE but if its disabled then inject undefined
	 * abort. This can happen when system implements SVE but Realms didn't
	 * ask for this feature during creation.
	 */
	...
	/*
	 * Realm used FPU or SVE register. Save NS state and restore realm simd
	 * state
	 */
#ifndef RMM_FPU_USE_AT_REL2
	simd_save_ns_state();
#endif
	rec_simd_restore_disable_trap(rec);
...
static bool handle_realm_rsi(struct rec *rec, struct rmi_rec_exit *rec_exit)
{
	...
	/* Ignore SVE hint bit, until RMM supports SMCCCv1.3 and SVE hint */
	 ...
	 */
	if (rsi_handler_needs_fpu(function_id) && rec_used_simd(rec)) {
