...
static bool handle_realm_rsi(struct rec *rec, struct rmi_rec_exit *rec_exit)
{
	...
	if ((rec->active_plane_id != PRIMARY_PLANE_ID) &&
	    !((function_id == SMC_RSI_HOST_CALL) && (plane->trap_hc == false))){
		/*
		 * SMC calls from secondary planes cause Plane exit due to
		 * synchronous exception.
		 *
		 * The only exception to this rule is SMC_RSI_HOST_CALL when
		 * the secondary plane is entered with trap_hc == RSI_NO_TRAP.
		 */
		advance_pc();
		return handle_secondary_plane_exit(rec, rec_exit,
						   ARM_EXCEPTION_SYNC_LEL);
