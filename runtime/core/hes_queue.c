...
static struct hes_attest_response_s hes_attest_response_array[MAX_CPUS] = { 0 };
...
void hes_attest_push_request_to_hes(void)
{
	...
	memset((void *)shared_buf, 0, rmm_el3_ifc_get_shared_buf_size());
	memcpy((void *)shared_buf, (void *)req, sizeof(*req));
	assert(req->version == 0x10);
...
void hes_attest_pull_response_from_hes(void)
{
	...
	struct hes_attest_response_s *hes_resp = &hes_attest_response_array[my_cpuid()];
