			   ...
			   unsigned long caller_sctlr_el1)
{
	...
	plane->sysregs.sctlr_el1 = SCTLR_EL1_FLAGS;

	/* Set the endianness of the target to that of the caller */
	plane->sysregs.sctlr_el1 |= caller_sctlr_el1 & SCTLR_ELx_EE_BIT;
}
					  ...
					  unsigned long status)
{
	...
	psci_reset_rec(&target_rec->plane_0, caller_sctlr_el1);
	target_rec->plane_0.regs[0] = context_id;
	target_rec->plane_0.pc = entry_point_address;
				    ...
				    struct rec *target_rec, unsigned long status)
{
	 ...
	 * keep it here as to document the API as well as for future checks.
	 */
	assert(calling_plane == &calling_rec->plane_0);
	...
	case SMC64_PSCI_CPU_ON:
		...
		rec_ret = complete_psci_cpu_on(target_rec,
						...
						calling_plane->sysregs.sctlr_el1,
						status);
		/*
