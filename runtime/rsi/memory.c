			      ...
			      struct rsi_result *res)
{
	...
	/* Only Primary Plane can isuue RSI calls */
			      ...
			      struct rsi_result *res)
{
	...
	/* Only Primary Plane can isuue RSI calls */
	assert(rec_active_plane(rec) == rec_primary_plane(rec));
	...
	if (*cookie == 0UL) {
		if (rtt_tree_pp) {
			*cookie = (base & GRANULE_MASK) |
				  (PRIMARY_S2_CTX_ID & ~GRANULE_MASK);
		}
		return true;
	}
...
void handle_rsi_mem_get_perm_value(struct rec *rec, struct rsi_result *res)
{
	...
	s2_ctx = plane_to_s2_context(rd, plane_id);
