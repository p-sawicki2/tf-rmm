					      ...
					      struct rsi_result *res)
{
	...
	uint8_t *attest_buf = rec->aux_data.attest_buffer;
	...
	if (attest_data->token_sign_ctx.copied_len == 0UL) {
		...
		attest_token_len = attest_cca_token_create(attest_buf,
					REC_ATTEST_BUFFER_SIZE,
		...
		attest_data->token_sign_ctx.token_len = attest_token_len;
	} else {
		attest_token_len = attest_data->token_sign_ctx.token_len;
	}

	length = (size < attest_token_len) ? size : attest_token_len;

	/* Copy attestation token */
	(void)memcpy(realm_att_token + offset,
		     attest_buf + attest_data->token_sign_ctx.copied_len,
		     length);
...
void handle_rsi_attest_token_init(struct rec *rec, struct rsi_result *res)
{
	...
	(void)memcpy(attest_data->token_sign_ctx.challenge, &rec->regs[1],
				      ...
				      struct rsi_result *res)
{
	...
	if (!GRANULE_ALIGNED(realm_buf_ipa) ||
	   (offset >= GRANULE_SIZE) || (offset + size > GRANULE_SIZE)) {
		res->smc_res.x[0] = RSI_ERROR_INPUT;
		return;
	}
