...
COMPILER_ASSERT(U(offsetof(struct rec_plane, sp_el0)) ==
	(U(offsetof(struct rec_plane, regs)) + U(sizeof(unsigned long) * RMM_REC_SAVED_GEN_REG_COUNT)));
 ...
 * (e.g. the most recently exited Plane or the place which the RMM is preparing
 * to enter to.
 */
