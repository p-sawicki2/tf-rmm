 ...
 */
static inline bool rec_mpidr_is_valid(unsigned long mpidr)
{
	return (mpidr & ~(MASK(RMI_MPIDR_AFF0) |
			  MASK(RMI_MPIDR_AFF1) |
			  MASK(RMI_MPIDR_AFF2) |
 ...
 */
static inline unsigned long rec_mpidr_to_idx(unsigned long mpidr)
{
	return (RMI_MPIDR_AFF(0, mpidr) |
		RMI_MPIDR_AFF(1, mpidr) |
		RMI_MPIDR_AFF(2, mpidr) |
		RMI_MPIDR_AFF(3, mpidr));
}
 ...
 * Aff3[39:32]:Aff2[23:16]:Aff1[15:8]:Aff0[3:0].
 */
static inline unsigned long rec_mpidr_to_mpidr(unsigned long mpidr)
{
	return (mpidr & (MASK(RMI_MPIDR_AFF0)	|
			 MASK(RMI_MPIDR_AFF1)	|
			 MASK(RMI_MPIDR_AFF2)))	|
		((mpidr & MASK(RMI_MPIDR_AFF3)) <<
