...
static bool complete_mmio_emulation(struct rec *rec, struct rmi_rec_enter *rec_enter)
{
	...
	/*
	 * If INJECT_SEA is set, we will only reach here if the conditions
	 * for that flag is satified and has an effect on the Realm viz to
			    ...
			    unsigned long rec_run_addr)
{
	 ...
	 * having an effect having an effect (Data Abort at Unprotected IPA)
	 * are a superset of those for RMI_EMULATED_MMIO (Data Abort at
	 * Unprotected IPA and access was an emulatable read).
	 */
