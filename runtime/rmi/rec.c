...
{
	unsigned long mdcr_el2_val = MDCR_EL2_INIT;
	...
	if (rd->pmu_num_cnts == 0U) {
		if (!is_feat_hpmn0_present()) {
			 ...
			 */
			mdcr_el2_val |= INPLACE(MDCR_EL2_HPMN, 1UL);
		}
	} else {
		mdcr_el2_val |= INPLACE(MDCR_EL2_HPMN, rd->pmu_num_cnts);
	}

	if (rd->pmu_enabled) {
		/* Disable trapping of accesses to PMU registers */
