...
static unsigned long realm_vtcr(struct rd *rd)
{
	...
	assert(((lpa2 == false) && (s2_starting_level >= S2TT_MIN_STARTING_LEVEL)) ||
	       ((lpa2 == true) && (s2_starting_level >= S2TT_MIN_STARTING_LEVEL_LPA2)));
...
static void rec_aux_granules_init(struct rec *r)
{
	...
	buffer_aux_granules_unmap(rec_aux, r->num_rec_aux);
}
