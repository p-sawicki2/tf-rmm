			     ...
			     unsigned long ulevel)
{
	...
	/*
	 * Lock the RTT root. Enformce locking order RD->RTT is enough to
	...
	parent_s2tte = s2tte_read(&s2_ctx, &parent_s2tt[wi.index]);
	...
	if (s2tte_is_unassigned_empty(&s2_ctx, parent_s2tte)) {
		 ...
		 */
		s2tte_write(&s2_ctx, &parent_s2tt[wi.index], 0UL);
	...
	s2tte_write(&s2_ctx, &parent_s2tt[wi.index], parent_s2tte);
		  ...
		  struct smc_result *res)
{
	...
	if (g_tbl->refcount == 0UL) {
		...
		s2tte = s2tte_read(&s2_ctx, &table[0]);
			 ...
			 struct smc_result *res)
{
	...
	s2tte = s2tte_read(&s2_ctx, &s2tt[wi.index]);
	...
	if (op == MAP_NS) {
		...
		s2tte_write(&s2_ctx, &s2tt[wi.index], s2tte);
				      ...
				      unsigned long s2tte)
{
	...
	if (!host_ns_s2tte_is_valid(&s2_ctx, s2tte, level)) {
		return RMI_ERROR_INPUT;
	}
 ...
 */
static int update_ripas(struct realm_s2_context *s2_ctx,
			unsigned long *s2ttep, long level,
			enum ripas ripas_val,
			enum ripas_change_destroyed change_destroyed)
{
	unsigned long pa, s2tte = s2tte_read(s2_ctx, s2ttep);
	int ret = 0;
	...
	s2tte_write(s2_ctx, s2ttep, s2tte);
	return ret;
}
			...
			struct smc_result *res)
{
	...
	for (index = wi.index; index < S2TTES_PER_S2TT; index++) {
		...
		s2tte = s2tte_read(&s2_ctx, &s2tt[index]);
		if (s2tte_is_unassigned_empty(&s2_ctx, s2tte)) {
			s2tte = s2tte_create_unassigned_ram(&s2_ctx);
			s2tte_write(&s2_ctx, &s2tt[index], s2tte);
		} else if (!s2tte_is_unassigned_ram(&s2_ctx, s2tte)) {
			break;
		}
				...
				struct smc_result *res)
{
	...
	unsigned long map_size = s2tte_map_size(s2_ctx, (int)level);
