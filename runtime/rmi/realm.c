 ...
 */
static void init_s2_starting_level(struct rd *rd, unsigned int plane_id)
{
	unsigned long current_ipa = 0U;
	struct s2tt_context *s2tt_ctx = s2_context(rd, plane_id);
...
static void init_overlay_permissions(struct rd *rd)
{
	...
	for (unsigned int plane_idx = 0U;
		...
		struct s2tt_context *s2_ctx = s2_context(rd, plane_idx);

		/* Default Access Permissions for Protected IPA space */
		default_perm = (plane_idx == PRIMARY_PLANE_ID) ?
		...
		rd->overlay_perms[plane_idx] = overlay_perm;
	}
			       ...
			       unsigned long realm_params_addr)
{
	...
	for (unsigned int i = 0U; i < realm_num_planes(rd); i++) {
		...
		s2tt_ctx->overlay_perm = &rd->overlay_perms[i];
	}
