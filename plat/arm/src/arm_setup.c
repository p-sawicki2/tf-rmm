...
void plat_setup(uint64_t x0, uint64_t x1, uint64_t x2, uint64_t x3)
{
	...
	/* We need at least one console and it needs to be PL011 */
	...
	uintptr_t uart_base = console_ptr->base;

	/* RMM currently only supports one console */
	ret = pl011_init(uart_base, console_ptr->clk_in_hz, console_ptr->baud_rate);
	if (ret != 0) {
		rmm_el3_ifc_report_fail_to_el3(E_RMM_BOOT_UNKNOWN_ERROR);
	}
