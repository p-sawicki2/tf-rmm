...
#define MINIMUM_LENGTH_FOR_FUZZ (HOST_MEM_SIZE * sizeof(unsigned char) \
            + RMM_MAX_GRANULES * sizeof(struct granule) + \
...
int main(int argc, char *argv[])
{
    ...
    while (__AFL_LOOP(10000)) {
        ...
        if(len < MINIMUM_LENGTH_FOR_FUZZ) continue;
        execute(buffer);
    }
#else
	if (argc < 2) return 1;

    // ALL pages, granule meta data, and a list of `FUZZ_COMMAND_COUNT` sequence calls.
	uint8_t buffer[MINIMUM_LENGTH_FOR_FUZZ] = { 0 };
