...
typedef void (*simd_vreg_wr_cb_t)(enum simd_variant variant, simd_vreg val,
								simd_vreg *reg);
...
/*
 * Structure to hold SIMD vreg access emulation data.
 */
struct simd_vreg_data {
	struct simd_vreg_cb callbacks;
...
/*
 * Sets up callbacks for access to SIMD registers.
 */
