 ...
 * callbacks. This function is meant to be invoked at a point wherein the
 * system is initialized properly and can restored for later test iterations.
 *
 * Arguments:
 ...
 * assumed to be saved prior to calling this API. This API should only be used
 * by the RMM runtime initialization stage, to restore the runtime to a known
 * state before starting any new test.
 *
 * Arguments:
 *	snapshot - Pointer to an array of struct sysreg_data where the
