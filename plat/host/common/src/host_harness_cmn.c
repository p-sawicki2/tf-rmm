				     ...
				     uint64_t c_size)
{
	...
	if (*buf_size < sizeof(platform_token)) {
		rmm_log("Failed to get platform token: Buffer is too small.\n");
		return -EINVAL;
	}

	(void)memcpy((void *)buf_pa, platform_token, sizeof(platform_token));
			   ...
			   uint64_t ecc_curve)
{
	...
	(void)memcpy((void *)buf_pa, sample_attest_priv_key,
			...
			struct smc_result *res)
{
	...
	switch(id) {
		...
		case SMC_RMM_GET_REALM_ATTEST_KEY:
			...
			break;
		default:
			rmm_log("Unimplemented monitor call id %lx\n", id);
	}
}
