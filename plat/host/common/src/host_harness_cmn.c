				     ...
				     uint64_t *remaining_len)
{
	...
	local_hunk_size = ATTEST_PLAT_TOKEN_HUNK_LEN;
	if (local_hunk_size > buf_size) {
		local_hunk_size = buf_size;
	}
	 ...
	 */
	if (local_hunk_size >= local_remaining_len) {
		local_hunk_size = local_remaining_len;
	}
	/* Update remaining bytes according to hunk size */
	local_remaining_len -= local_hunk_size;

	(void)memcpy((void *)buf_pa,
		(void *)platform_token + platform_token_offset,
		local_hunk_size);

	platform_token_offset += local_hunk_size;
	*token_hunk_size = local_hunk_size;
