...
static struct granule *init_aux_page(void)
{
	...
	__CPROVER_assume(STATE(&g) == GRANULE_STATE_REC_AUX);
...
struct granule *init_rec_page(struct granule *g_rd)
{
	...
	__CPROVER_assume(STATE(&g) == GRANULE_STATE_REC);
	...
	__CPROVER_assert(STATE(g_rd) == GRANULE_STATE_RD,
...
struct rmm_rec Rec(uint64_t addr)
{
	...
	struct rmm_rec spec_rec = {
		...
		.state = REFCOUNT(rec_ptr->g_rec) == 0 ? REC_READY:REC_RUNNING,
...
bool AuxStateEqual(struct granule **aux, uint64_t num_aux, unsigned char state)
{
	...
	unsigned char aux_state = STATE(g_rec_aux);
	...
	for (int i = 0; i < num_aux && i < MAX_REC_AUX_GRANULES; ++i) {
		if (!PaIsDelegable(granule_metadata_ptr_to_pa(aux[i])) || STATE(aux[i]) != state) {
			return false;
		}
	}
	return true;
