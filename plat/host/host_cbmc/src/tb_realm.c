...
struct rmm_realm Realm(uint64_t addr)
{
	...
	s2_ctx = primary_s2_context(rd_ptr);
...
bool valid_rd(struct rd value)
{
	return valid_realm_state(value.state)
		&& valid_s2tt_context(*primary_s2_context(&value))
...
struct rd init_rd(void)
{
	...
	struct s2tt_context *s2_ctx = primary_s2_context(&value);
