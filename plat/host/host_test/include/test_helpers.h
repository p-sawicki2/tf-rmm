 ...
 * test will fail and probably abort. If we expect another assertion, then
 ...
 * test will fail and probably abort. If we expect another panic, then
 * this API must be called again to indicate that. It is recommended to setup
 * the expected panic() behavior during TEST_SETUP().
 *
 ...
 * This is preferred than fail with FAIL_TEST() as this can cause
 * random aborts.
 */
