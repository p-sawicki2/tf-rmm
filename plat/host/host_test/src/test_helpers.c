...
static void setup_sysreg_and_boot_manifest(void)
{
	 ...
	 * range of 18 bits
	 */
	(void)host_util_set_default_sysreg_cb("id_aa64mmfr0_el1", true,
	 ...
	 */
	(void)host_util_set_default_sysreg_cb("ich_vtr_el2", false,
			INPLACE(ICH_VTR_EL2_PRE_BITS, 5UL));

	/* Used to hold CPU Id. Reset to CPUid 0. */
	(void)host_util_set_default_sysreg_cb("tpidr_el2", true, 0UL);

	(void)host_util_set_default_sysreg_cb("sctlr_el2", true, 0UL);
