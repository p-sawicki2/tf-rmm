...
#define TOTAL_MMAP_REGIONS	(COMMON_REGIONS + PLAT_MAX_MMAP_REGIONS)
		   ...
		   unsigned int nregions)
{
	...
	if (nregions > PLAT_MAX_MMAP_REGIONS) {
		return -ERANGE;
	}
	...
	if (nregions > 0U) {
		 ...
		 */
		if (plat_regions[0U].base_va < RMM_CODE_START) {
			memcpy((void *)&static_regions[0U],
			       (void *)&plat_regions[0U],
			       sizeof(struct xlat_mmap_region) * nregions);
			memcpy((void *)&static_regions[nregions],
			       (void *)&regions[0],
			...
			memcpy((void *)&static_regions[0U],
			       ...
			       sizeof(struct xlat_mmap_region) *
							COMMON_REGIONS);
			memcpy((void *)&static_regions[COMMON_REGIONS],
			       (void *)&plat_regions[0U],
			       sizeof(struct xlat_mmap_region) * nregions);
		}
	} else {
