		   ...
		   struct xlat_mmap_region *plat_regions)
{
	...
	ret = xlat_ctx_allocate_tables((uintptr_t)&runtime_xlat_buf[0],
					   ...
					   &runtime_tbls);
	if (ret != 0) {
		ERROR("%s (%u): Failed to allocate tables for the runtime translation context\n",
			__func__, __LINE__);
		return ret;
	}
	...
	if (ret != 0) {
		ERROR("%s (%u): Failed to register the xlat context within the xlat library\n",
			__func__, __LINE__);
		return ret;
	}
