...
bool valid_realm_state(unsigned long value)
{
	return value == (unsigned long)REALM_STATE_NEW
	       || value == (unsigned long)REALM_STATE_ACTIVE
	       || value == (unsigned long)REALM_STATE_SYSTEM_OFF;
}
...
{
    struct rd * realm = granule_metadata_ptr_to_buffer_ptr(g_rd);
    return realm->s2_ctx.s2_starting_level;
}
...
{
	return algo == HASH_ALGO_SHA256 || algo == HASH_ALGO_SHA512;
}
