...
struct rmi_realm_params_buffer RealmParams(uint64_t addr)
{
	...
	struct rmi_realm_params_buffer spec_realmparam = {
        .s2sz = EXTRACT(RMM_FEATURE_REGISTER_0_S2SZ, params_ptr->features_0),
        .sve_vl = EXTRACT(RMM_FEATURE_REGISTER_0_SVE_VL, params_ptr->features_0),
        .pmu_num_ctrs = EXTRACT(RMM_FEATURE_REGISTER_0_PMU_NUM_CTRS, params_ptr->features_0),
...
bool RmiRealmParamsIsValid(uint64_t params_addr)
{
    ...
    return validate_feature_register(RMM_FEATURE_REGISTER_0_INDEX, realm_params_buf->features_0);
}
...
{
    return Realm(rd_addr).rec_index;
}
