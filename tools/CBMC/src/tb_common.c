...
uint64_t pow(uint64_t base, uint64_t power)
{
	...
	if (base == 2) {
		return UINT64_C(1) << power;
	}
	uint64_t rst = base;

	for (uint64_t i = UINT64_C(1); i < power; ++i) {
		rst *= base;
	}
	return rst;
}
...
{
	/*
	 * NOTE: the explicit pointer to integer type cast is necessary, as CBMC check fails
	 * without it.
	 */
	if (GRANULE_ALIGNED(addr) && (uint64_t)granules_buffer <= addr &&
		addr < (uint64_t)granules_buffer + sizeof(granules_buffer)) {
		/*
		 * Keep these assserts for sanitary check, there was situation these asserts fail
		 * possibly due to CBMC dislike type conversion between number and pointer
		 */
...
struct granule *inject_granule_at(const struct granule *granule_metadata,
				  const void *physical_page,
				  size_t size_physical_page,
				  size_t index)
{
