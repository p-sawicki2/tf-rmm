...
bool valid_granule(struct granule value) {
    ...
    case GRANULE_STATE_RTT: return value.refcount >= 0 && value.refcount <= GRANULE_SIZE / sizeof(uint64_t);
...
{
	return addr >= (uint64_t)granules_buffer && addr < (uint64_t)granules_buffer + sizeof(granules_buffer);
}
