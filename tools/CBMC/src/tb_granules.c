...
unsigned long nondet_granule_index(void)
{
...
bool valid_granule(struct granule value) {
    ...
    /*if (!valid_granule_state(value.state)) {*/
        /*return false;*/
    /*}*/
    switch (value.state) {
    ...
    case GRANULE_STATE_RTT: return value.refcount >= 0 && value.refcount <= 512UL;
...
{
	return addr >= (unsigned long)granules_buffer && addr < (unsigned long)granules_buffer + sizeof(granules_buffer);
}
...
struct SPEC_granule Granule(uint64_t addr)
{
	...
	switch (result->state) {
        case GRANULE_STATE_NS:
		spec_granule.pas = PHYSICAL_ADDRESS_SPACE_NS;
		spec_granule.owner = 0UL;
		break;
        case GRANULE_STATE_RD:
		spec_granule.pas = PHYSICAL_ADDRESS_SPACE_REALM;
		/* TODO
		 * this looks incorrect, we are looking for the owner == realm
		 */
		spec_granule.owner = granule_addr(result);
		break;
	case GRANULE_STATE_DATA:
		/*
		 * the owner on this granule should be a RD....
		 */
		spec_granule.pas = PHYSICAL_ADDRESS_SPACE_REALM;
		spec_granule.owner = nondet_unsigned_long();
		break;
	...
	case GRANULE_STATE_DELEGATED:
		...
		break;
        default:
		spec_granule.pas = PHYSICAL_ADDRESS_SPACE_SECURE;
		spec_granule.owner = 0UL;
	}
