...
// Global lookup table.
unsigned long entry_values[5] = { 0 };
unsigned long entry_to_phys[5] = { 0 };
size_t next_available_entry_map = 0;

// A function CBMC shadow
unsigned long table_entry_to_phys(unsigned long entry)
{
    for(int i = 0; i < sizeof(entry_values)/sizeof(unsigned long) && i < next_available_entry_map; i++)
...
bool valid_entry_value(unsigned long entry_value, int level)
{
	...
	unsigned long desc_ripas = entry_value & S2TTE_INVALID_RIPAS_MASK;
...
struct granule * init_walk_path(struct granule * g_root, int start_level, unsigned long max_level)
{
    ...
    for(int cur_level = start_level; cur_level <= max_level; cur_level++) {
        ...
        // We assume there are at least one unused slot.
        __CPROVER_assume(next_rtt.refcount <= 511);
        ...
        unsigned long addr = granule_metadata_ptr_to_pa(g_next);
        unsigned long * prev_rtt_content = granule_metadata_ptr_to_buffer_ptr(g_cur_rtt);
        unsigned long index = nondet_unsigned_long();
        __CPROVER_assume(index >= 0 && index < NB_RTT_GRANULE);
        unsigned long entry_st2tte_value = nondet_unsigned_long();
...
bool read_entry_from_s2tte(unsigned long s2tte, int level, struct rmm_rtt_entry *entry) 
{
    ...
    */
	...
	if (s2tte_is_unassigned(s2tte)) {
		...
		entry->ripas = (unsigned long)ripas;
	} else if (s2tte_is_destroyed(s2tte)) {
			       ...
			       unsigned long level) 
{
    ...
    unsigned long ipa_bits = rd_ptr->s2_ctx.ipa_bits;
    unsigned int rtt_num_start = rd_ptr->s2_ctx.num_root_rtts;

    // deal with the concatenated rtt.
    unsigned long sl_idx = s2_sl_addr_to_idx(ipa, rst.level, ipa_bits);
    ...
    for(; rst.level < level; ++rst.level) {
        unsigned long idx = s2_addr_to_idx(ipa, rst.level);

        // Get the table.
        unsigned long *table = granule_metadata_ptr_to_buffer_ptr(rst.rtt_addr);

        unsigned long pa = table_entry_to_phys(table[idx]);
        if (!valid_pa(pa)) {
            break;
        }
    ...
    // Extract entry state
    unsigned long entry_value = table[idx];
	unsigned long s2tte = s2tte_read(&entry_value);
