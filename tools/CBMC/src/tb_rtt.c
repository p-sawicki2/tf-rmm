...
#define MAX_RTT_LOOKUP_TABLE 5

// Global lookup table.
struct rtt_lookup_table_entry {
    uint64_t value;
    uint64_t phy;
} rtt_lookup_table[MAX_RTT_LOOKUP_TABLE];
...
{
    for(int i = 0; i < MAX_RTT_LOOKUP_TABLE && i < next_available_entry_map; i++)
        if(rtt_lookup_table[i].value == entry)
            return rtt_lookup_table[i].phy;
...
bool valid_entry_value(uint64_t entry_value, int level)
{
    ...
    for(size_t i = 0; i < 5; i++)
        if(entry_value == rtt_lookup_table[i].value) return false;
    ...
    // if the next level is still a table
    if (level + 1 <= RTT_PAGE_LEVEL && (entry_value & DESC_TYPE_MASK) != S2TTE_L012_TABLE)
        return false;

	if (!(s2tte_is_unassigned_empty(entry_value)
	      || s2tte_is_unassigned_ram(entry_value)
	      || s2tte_is_unassigned_destroyed(entry_value)
	      || s2tte_is_assigned_empty(entry_value, level)
	      || s2tte_is_assigned_ram(entry_value, level)
	      || s2tte_is_assigned_destroyed(entry_value, level)
	      || s2tte_is_unassigned_ns(entry_value)
	      || s2tte_is_assigned_ns(entry_value, level)
...
struct granule * init_walk_path(struct granule * g_root, int start_level, uint64_t max_level)
{
    ...
    for(int cur_level = start_level; cur_level <= max_level; cur_level++) {
        ...
        // We assume there are at least one unused slot hence the refcount should not reach maximum value.
        ...
        rtt_lookup_table[next_available_entry_map].value = entry_st2tte_value;
        rtt_lookup_table[next_available_entry_map++].phy = addr;

        // link the child rtt to the parent rtt
        prev_rtt_content[index] = entry_st2tte_value;

        // Inc the ref count of the old cur_rtt. 
        ++(g_cur_rtt->refcount);
        __CPROVER_assume(g_cur_rtt->refcount <= (GRANULE_SIZE / sizeof(uint64_t)) - 1);

        // add into the lock relation for lock discipline check
        add_lock_relation(g_cur_rtt, g_next);
...
bool read_entry_from_s2tte(uint64_t s2tte, int level, struct rmm_rtt_entry *entry) 
{
    *entry = RttEntryFromDescriptor(s2tte);
	...
	if (s2tte_is_unassigned_empty(s2tte)) {
		...
		entry->state = RMI_UNASSIGNED;
		entry->ripas = RIPAS_EMPTY;
	} else if (s2tte_is_assigned_ns(s2tte, level)) {
		entry->state = RMI_ASSIGNED;
		entry->ripas = RIPAS_EMPTY;
...
uint64_t RttSkipEntriesUnlessState(struct SPEC_rtt rtt, uint64_t level, uint64_t ipa, enum SPEC_rtt_state state) 
{
    //TODO
    return 0;
}
...
uint64_t RttEntryState(enum SPEC_rtt_state rtt_state)
{
    switch(rtt_state) {
    case UNASSIGNED: 
    case UNASSIGNED_NS: return RMI_UNASSIGNED;
    case ASSIGNED:
    case ASSIGNED_NS: return RMI_ASSIGNED;
    case DESTROYED: return RMI_DESTROYED;
    case TABLE: return RMI_TABLE;
    }
    UNREACHABLE;
}
