...
void __init_global_state(unsigned long cmd) {
	...
	switch(cmd) {
        ...
        // One for the new rec and two for the potentially continuous rec_aux
        init_granule_and_page();
        init_granule_and_page();
        init_granule_and_page();
		init_realm_descriptor_page();
        ...
        init_rec_run_page();
        // NOT return here, but continue to initial other states.
        }
        ...
        // Initial an new granule for the new rtt or the mapped page.
		init_granule_and_page();
		init_granule_and_page();
        // NOT return here, but continue to initial other states.
        ...
        // Initial a realm 
		struct granule* g_rd = init_realm_descriptor_page();
        // Initial the root rtt to the realm 
        ...
        // Initial ONE valid walk up to `level`
...
void tb_handle_smc(struct tb_regs* config) {
    ...
    case SMC_RMM_DATA_DESTROY:
		...
		smc_data_destroy(config->X1, config->X2, &rst);
		result = rst.x[0];
        config->X1 = rst.x[1];
        config->X2 = rst.x[2];
        break;
    ...
    case SMC_RMM_RTT_UNMAP_UNPROTECTED:
		struct smc_result rst;
        smc_rtt_unmap_unprotected(config->X1, 
                       config->X2, 
                       config->X3,
				       &rst);
		result = rst.x[0];
