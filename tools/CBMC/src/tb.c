...
void __init_global_state(unsigned long cmd) {
	...
	switch(cmd) {
        ...
        init_rtt_root_page(num_root_rtts);
        ...
        int start_level = start_level_from_realm_descriptor(g_rd);
        unsigned long level = nondet_unsigned_long();
        __CPROVER_assume(valid_max_walk_path_level(start_level, level));
        init_walk_path(g_root_rtt, start_level, level);
		return;
        }
        case SMC_RMM_DATA_CREATEUNKNOWN: {
		init_realm_descriptor_page();
        //TODO
