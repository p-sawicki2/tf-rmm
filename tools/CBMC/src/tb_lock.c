    ...
    if (after.has_parent) {
        ...
        for(int prev = index - 1; prev >= 0; prev--) {
            ...
            for(int i = 0; i < next_relation_entry && i < MAX_LOCK_RELATION; i++) {
                if( after.is_lock
                        && lock_relation[i].child == after.granule_addr
                        && lock_relation[i].parent == before.granule_addr ) return true;
            }
        }
        // No parent is found.
        return false;
...
{
    lock_relation[next_relation_entry].parent = before;
    lock_relation[next_relation_entry++].child = after;
