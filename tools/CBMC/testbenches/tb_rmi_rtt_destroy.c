    ...
    int64_t level)
{
    ...
    bool success_ripas_post = walk.entry.ripas == DESTROYED;
    ...
    bool prop_success_ripas_ante = no_failures_pre;
    __COVER(prop_success_ripas_ante);
    if (prop_success_ripas_ante) {
        bool prop_success_ripas_cons = success_ripas_post && (result == RMI_SUCCESS);
        __COVER(prop_success_ripas_cons);
        __ASSERT(prop_success_ripas_cons, "prop_success_ripas_cons");
    }
