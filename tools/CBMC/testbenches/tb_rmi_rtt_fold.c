    ...
    int64_t level)
{
    ...
    entry_idx = RttEntryIndex(
        ipa, walk.level);
    fold = RttFold(Rtt(walk.entry.addr));
    ...
    bool success_rtte_addr_pre = (fold.state != UNASSIGNED && fold.state != UNASSIGNED_NS && fold.state != DESTROYED);
...
void entry_point(
    void)
{
    tb_rmi_rtt_fold(nondet_uint64_t(), nondet_uint64_t(), nondet_int64_t());
}
