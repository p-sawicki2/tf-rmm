    ...
    int64_t level)
{
    ...
    bool success_state_invalid_pre = (walk.entry.state == UNASSIGNED || walk.entry.state == UNASSIGNED_NS || walk.entry.state == DESTROYED);
    ...
    walk = RttWalk(
        rd, ipa, level);
    unsigned long walk_level = __tb_regs.X1;
    unsigned long state = __tb_regs.X2;
    unsigned long desc = __tb_regs.X3;
    unsigned long X3 = __tb_regs.X3;
    unsigned long X4 = __tb_regs.X4;
    rtte = RttEntryFromDescriptor(desc);
    ...
    bool success_state_invalid_post = X3 == Zeros();
    ...
    bool success_ripas_unprot_post = X4 == Zeros();
...
void entry_point(
    void)
{
    tb_rmi_rtt_read_entry(nondet_uint64_t(), nondet_uint64_t(), nondet_uint64_t());
}
