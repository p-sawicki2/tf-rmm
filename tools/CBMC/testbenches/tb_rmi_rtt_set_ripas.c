    ...
    uint64_t top)
{
    ...
    __tb_regs.X3 = (uint64_t)base;
    __tb_regs.X4 = (uint64_t)top;
    ...
    bool failure_base_bound_pre = base != Rec(rec).ripas_addr;
    bool failure_top_bound_pre = UInt(top) > UInt(Rec(rec).ripas_top);
    ...
    bool failure_base_bound_post = ResultEqual_2(result, RMI_ERROR_INPUT);
    ...
    bool success_rtte_ripas_post = RttEntriesInRangeRipas(Rtt(walk.rtt_addr), walk.level, base, walk_top, Rec(rec).ripas_value);
    bool success_ripas_addr_post = Rec(rec).ripas_addr == walk_top;
    ...
    bool prop_failure_base_bound_ante = !failure_rd_align_pre
        && !failure_rd_bound_pre
        && !failure_rd_state_pre
        && !failure_rec_align_pre
        && !failure_rec_bound_pre
        ...
        && !failure_size_valid_pre
        && failure_base_bound_pre;
    __COVER(prop_failure_base_bound_ante);
    if (prop_failure_base_bound_ante) {
        bool prop_failure_base_bound_cons = failure_base_bound_post;
        __COVER(prop_failure_base_bound_cons);
        __ASSERT(prop_failure_base_bound_cons, "prop_failure_base_bound_cons");
    }
