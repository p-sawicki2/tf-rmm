...
find_program(CLANG-TIDY_EXECUTABLE "run-clang-tidy"
...
function(clangtidy_get_stats stats_arg warnings_ret errors_ret)
  ...
  foreach(output_line IN LISTS output_lines)
    string(FIND "${output_line}" "warning: " idx)

    if(NOT ${idx} EQUAL -1)
      MATH(EXPR warnings "${warnings} + 1")
    endif()

    string(FIND "${output_line}" "error: " idx)
...
function(run_clangtidy source_files warnings_ret errors_ret)
  ...
  foreach(source_file ${source_files})
    ...
    #
    # Run clang-tidy on each file, one at a time.
    #
    # We pass the compile commands database (in the build/ folder) to
    # clang-tidy, which records the compile options used in the build.
    #
...
if(CLANG-TIDY_CODEBASE)
  ...
  if(NOT EXISTS "${compile_commands}")
    message(FATAL_ERROR
      ...
      "`-DCMAKE_EXPORT_COMPILE_COMMANDS=1` during configuration.")
  endif()
