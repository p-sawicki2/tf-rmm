...
/* Function to get a random granule index in the range [1, NR_GRANULES - 1) */
static inline unsigned int get_rand_granule_idx(void)
{
	return (unsigned int)test_util_get_rand_in_range(1,
...
/*
 * Function to get a random granule address outside the valid range.
 * The address will be aligned to GRANULE_SIZE.
 *
 * If no address is found, the function will return false.
 */
static bool get_out_of_range_granule(unsigned long *addr, bool higher_range)
{
	...
	if (higher_range == true) {
		*addr = (unsigned long)(test_util_get_rand_in_range(
		...
		*addr -= (granules_below == 1U ?
			    GRANULE_SIZE :
			    GRANULE_SIZE * test_util_get_rand_in_range(1,
							granules_below - 1U));
	}
 ...
 * This function relies on add_to_granule() API to be validated.
 */
...
TEST(granule, granule_refcount_read_relaxed_TC1)
{
	...
	unsigned long val = (unsigned long)test_util_get_rand_in_range(10, INT_MAX);
...
TEST(granule, granule_refcount_read_acquire_TC1)
{
	struct granule *granule;
	unsigned long addr = get_rand_granule_addr();
	unsigned long val = (unsigned long)test_util_get_rand_in_range(10, 10000);
	unsigned long read_val;
...
TEST(granule, find_granule_TC2)
{
	...
	address += test_util_get_rand_in_range(1, GRANULE_SIZE - 1);
	granule = find_granule(address);
...
TEST(granule, find_lock_two_granules_TC1)
{
	...
	do {
		g1_index = test_util_get_rand_in_range(1,
					test_helper_get_nr_granules() - 1);
		g2_index = test_util_get_rand_in_range(1,
					test_helper_get_nr_granules() - 1);
...
TEST(granule, find_lock_two_granules_TC3)
{
	...
	tmp_addr = addr2 + test_util_get_rand_in_range(1, GRANULE_SIZE - 1);
...
TEST(granule, find_lock_granule_TC3)
{
	 ...
	 ***************************************************************/
	addr = get_rand_granule_addr();
	addr += test_util_get_rand_in_range(1, GRANULE_SIZE - 1);
...
TEST(granule, granule_put_TC2)
{
	 ...
	 * The refcount before the test starts is expected to be 0.
	 ******************************************************************/
	get_count = (unsigned int)test_util_get_rand_in_range(10, 1000);
...
TEST(granule, granule_refcount_inc_TC1)
{
	unsigned long address = get_rand_granule_addr();
	struct granule *granule = find_granule(address);
	unsigned long val = (unsigned long)test_util_get_rand_in_range(1, INT_MAX);
	...
	for (unsigned int i = 0U; i < 3U; i++) {
		...
		for (unsigned int j = 0U; j < MAX_CPUS; j++) {
			...
			for (unsigned int k = 0; k < NR_CPU_SLOTS; k++) {
				...
				memset((void *)addrs[i],
					test_util_get_rand_in_range(1, INT_MAX),
								GRANULE_SIZE);
