...
static inline unsigned int get_rand_granule_idx(void)
{
	return (unsigned int)test_helpers_get_rand_in_range(1UL,
			(unsigned long)(test_helpers_get_nr_granules() - 2U));
...
static bool get_out_of_range_granule(unsigned long *addr, bool higher_range)
{
	...
	if (higher_range == true) {
		*addr = (test_helpers_get_rand_in_range(
			(unsigned long)test_helpers_get_nr_granules(),
			(unsigned long)(test_helpers_get_nr_granules() + 10)) *
								GRANULE_SIZE);
		...
		*addr -= (granules_below == 1U ?
			    ...
			    GRANULE_SIZE * test_helpers_get_rand_in_range(1UL,
					(unsigned long)(granules_below - 1U)));
	}
...
{
	unsigned int lock =
		(unsigned int)test_helpers_get_rand_in_range(1UL,
...
ASSERT_TEST(granule, addr_to_granule_TC3)
{
	...
	addr += test_helpers_get_rand_in_range(1UL,
					(unsigned long)(GRANULE_SIZE - 2));
	test_helpers_expect_assert_fail(true);
...
TEST(granule, granule_refcount_read_relaxed_TC1)
{
	...
	unsigned long addr = get_rand_granule_addr();
	unsigned long val =
		(unsigned long)test_helpers_get_rand_in_range(10UL,
						(unsigned long)INT_MAX);
...
TEST(granule, find_granule_TC2)
{
	...
	address += test_helpers_get_rand_in_range(1UL,
				(unsigned long)(GRANULE_SIZE - 1));
	granule = find_granule(address);
...
TEST(granule, find_lock_two_granules_TC1)
{
	...
	do {
		g1_index = (int)test_helpers_get_rand_in_range(1UL,
			(unsigned long)(test_helpers_get_nr_granules() - 1));
		g2_index = (int)test_helpers_get_rand_in_range(1,
			(unsigned long)(test_helpers_get_nr_granules() - 1));
...
TEST(granule, find_lock_two_granules_TC3)
{
	...
	tmp_addr = addr2 + test_helpers_get_rand_in_range(1UL,
					(unsigned long)(GRANULE_SIZE - 1));
...
TEST(granule, granule_refcount_inc_TC1)
{
	unsigned long address = get_rand_granule_addr();
	struct granule *granule = find_granule(address);
	unsigned long val = test_helpers_get_rand_in_range(1UL,
						(unsigned long)INT_MAX);
	unsigned int lock = set_rand_non_zero_lock_value(granule);
...
TEST(granule, granule_refcount_dec_TC1)
{
	unsigned long address = get_rand_granule_addr();
	struct granule *granule = find_granule(address);
	unsigned long val = test_helpers_get_rand_in_range(10UL,
						(unsigned long)INT_MAX);
	unsigned int lock = set_rand_non_zero_lock_value(granule);
...
TEST(granule, find_lock_unused_granule_TC1)
{
	 ...
	 *	- Test with an unused granule in the same state as used on
	...
	for (unsigned int i = 0U; i < 3U; i++) {
		...
		/* Repeat the test, this time, 'refcout' is != 0 */
	...
	for (unsigned int i = 0U; i < 3U; i++) {
		...
		for (unsigned int j = 0U; j < MAX_CPUS; j++) {
			...
			for (unsigned int k = 0; k < NR_CPU_SLOTS; k++) {
				...
				memset((void *)addrs[i],
					(int)test_helpers_get_rand_in_range(1UL,
						(unsigned long)INT_MAX),
						GRANULE_SIZE);
