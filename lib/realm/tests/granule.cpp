...
TEST_GROUP(granule) {
	...
	TEST_SETUP()
	{
		...
		test_helper_expect_assert_fail(false);
		test_helper_expect_panic(false);
	}
...
TEST(granule, addr_to_granule_assert_TC1)
{
	/******************************************************************
	 * TEST CASE 2:
	 *
	...
	test_helper_expect_assert_fail(true);
	(void)addr_to_granule((unsigned long)NULL);
	test_helper_fail_if_no_assert_failed();
}

TEST(granule, addr_to_granule_assert_TC2)
{
	...
	addr += get_rand_in_range(1, GRANULE_SIZE - 2);
	test_helper_expect_assert_fail(true);
	(void)addr_to_granule(addr);
	test_helper_fail_if_no_assert_failed();
}

TEST(granule, addr_to_granule_assert_TC3)
{
...
TEST(granule, addr_to_granule_assert_TC4)
{
	unsigned long addr;
...
TEST(granule, granule_addr_assert_TC1)
{
	/******************************************************************
	 * TEST CASE 2:
	 *
...
TEST(granule, granule_addr_assert_TC2)
{
	struct granule *granule;
...
TEST(granule, granule_addr_assert_TC3)
{
	struct granule *granule;
...
TEST(granule, find_lock_two_granules_assert_TC1)
{
	struct granule *granule;
	unsigned long addr1, addr2;
...
TEST(granule, find_lock_two_granules_assert_TC2)
{
	struct granule *granule;
	unsigned long addr1, addr2;
...
TEST(granule, granule_lock_assert_TC1)
{
	struct granule *granule;
	...
	granule = addr_to_granule(addr);
	do {
		state = get_rand_in_range((int)GRANULE_STATE_NS,
					  (int)GRANULE_STATE_LAST);
		expected = get_rand_in_range((int)GRANULE_STATE_NS,
					     (int)GRANULE_STATE_LAST);
...
TEST(granule, granule_refcount_dec_assert_TC1)
{
	unsigned long address = get_rand_granule_addr();
	struct granule *granule = find_granule(address);
	unsigned long val = (unsigned long)get_rand_in_range(10, INT_MAX - 1);
...
TEST(granule, atomic_granule_put_release_assert_TC1)
{
	unsigned long address = get_rand_granule_addr();
	struct granule *granule = find_granule(address);
...
TEST(granule, granule_memzero_assert_TC1)
{
	/***************************************************************
	 * TEST CASE 2:
	 *
