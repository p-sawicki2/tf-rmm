...
void s2tte_create_assigned_empty_tc1(void)
{
	...
	for (long i = s2tt_test_helpers_min_block_lvl();
		...
		CHECK_TRUE(s2tt_test_helpers_s2tte_to_pa(tte, i) == pa);

		/* Validate the RIPAS */
		CHECK_TRUE((tte & S2TTE_INVALID_RIPAS_MASK) ==
						S2TTE_INVALID_RIPAS_EMPTY);

		/* Validate the HIPAS */
		CHECK_TRUE((tte & S2TTE_INVALID_HIPAS_MASK) ==
						S2TTE_INVALID_HIPAS_ASSIGNED);

		/* The rest of the fields must be all 0 */
		CHECK_TRUE((tte & ~(s2tt_test_helpers_s2tte_oa_mask(i) |
...
void s2tte_create_assigned_empty_tc2(void)
{
	...
	pa += test_helpers_get_rand_in_range(1UL, (unsigned long)GRANULE_SIZE);
...
void s2tte_create_assigned_ram_tc1(void)
{
	...
	for (long i = s2tt_test_helpers_min_block_lvl();
		...
		CHECK_TRUE(s2tt_test_helpers_s2tte_to_attrs(tte, false)
								== attrs);
...
void s2tte_create_assigned_ns_tc1(void)
{
	...
	for (long i = s2tt_test_helpers_min_block_lvl();
		...
		/* Validate the attributes */
		CHECK_TRUE(s2tt_test_helpers_s2tte_to_attrs(tte, true)
						== (attrs | host_attrs));
...
void s2tte_create_assigned_destroyed_tc1(void)
{
	...
	for (long i = s2tt_test_helpers_min_block_lvl();
		...
		CHECK_TRUE((tte & S2TT_TEST_DESC_TYPE_MASK) ==
...
void s2tte_create_table_tc1(void)
{
	     ...
	     level < S2TT_TEST_HELPERS_MAX_LVL; level++)
	{
		...
		/* Validate the address */
		CHECK_TRUE(s2tt_test_helpers_s2tte_to_pa(tte, level) == pa);

		/* Validate the descriptor type */
		CHECK_TRUE(EXTRACT(S2TT_TEST_DESC_TYPE, tte) == S2TT_TEST_TABLE_DESC);
...
void host_ns_s2tte_tc1(void)
{
	...
	for (long level = s2tt_test_helpers_min_block_lvl();
		...
		/* Validate the address */
		CHECK_TRUE(s2tt_test_helpers_s2tte_to_pa(val_tte, level) ==
...
void s2tte_get_ripas_tc1(void)
{
	     ...
	     ripas <= S2TT_TEST_RIPAS_DESTROYED; ripas++)
	{
		/* HIPAS = UNASSIGNED */
		tte = unassigned_cbs[ripas]();
		CHECK_TRUE((unsigned int)s2tte_get_ripas(tte) == ripas);
		     ...
		     level <= S2TT_TEST_HELPERS_MAX_LVL; level++)
		{
			...
			if (ripas != S2TT_TEST_RIPAS_RAM) {
				pa = s2tt_test_helpers_gen_pa(level, true);
				tte = assigned_cbs[ripas](pa, level);
				CHECK_TRUE((unsigned int)s2tte_get_ripas(tte) ==
									ripas);
			}
		}
	}
...
void s2tt_init_assigned_empty_tc1(void)
{
	...
	for (long level = s2tt_test_helpers_min_block_lvl();
		...
		for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
			...
			pa += i * s2tte_map_size(level);
			s2tte =	s2tte_create_assigned_empty(pa, level);

			CHECK_TRUE(s2tte = s2tt[i]);
		}
	}
}
...
void s2tt_init_assigned_empty_tc5(void)
{
	...
	unsigned long pa = s2tt_test_helpers_gen_pa(level, true) +
		test_helpers_get_rand_in_range(1UL, GRANULE_SIZE);
...
void s2tte_pa_tc1(void)
{
	...
	for (long level = s2tt_test_helpers_min_table_lvl();
		...
		CHECK_TRUE(pa == s2tte_pa(tte, level));
	}
}
...
void addr_is_level_aligned_tc2(void)
{
	...
	for (long level = s2tt_test_helpers_min_table_lvl();
	     level <= S2TT_TEST_HELPERS_MAX_LVL; level++) {
		unsigned long pa = s2tt_test_helpers_gen_pa(level, true);
		pa += test_helpers_get_rand_in_range(1UL, GRANULE_SIZE);
...
void s2tte_map_size_tc1(void)
{
	...
	for (long level = s2tt_test_helpers_min_table_lvl();
		...
		CHECK_TRUE(expected_size == s2tte_map_size(level));
	}
}
...
void table_maps_assigned_empty_block_tc1(void)
{
	...
	for (long level = s2tt_test_helpers_min_block_lvl();
		...
		unsigned long s2tt[S2TTES_PER_S2TT];
		unsigned long pa = s2tt_test_helpers_gen_pa(level - 1UL, true);
...
void rtt_walk_lock_unlock_tc1(void)
{
	...
	CHECK_TRUE(end_level == wi.last_level);
	CHECK_TRUE(val_granule == wi.g_llt);
	CHECK_TRUE(idx_per_lvl[end_level] == wi.index);
	...
	CHECK_TRUE((end_level - 1L) == wi.last_level);
	CHECK_TRUE(val_granule == wi.g_llt);
	CHECK_TRUE(idx_per_lvl[end_level - 1L] == wi.index);
