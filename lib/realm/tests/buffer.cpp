...
TEST(slot_buffer, granule_map_buffer_unmap_TC1)
{
	...
	cb.buffer_map = test_buffer_map_ret_va;
	(void)test_helpers_register_cb(cb, CB_BUFFER_MAP);
	cb.buffer_unmap = test_buffer_unmap_from_va;
	...
	for (unsigned int i = 0U; i < MAX_CPUS; i++) {
		...
		for (unsigned int j = 0U; j < NR_CPU_SLOTS; j++) {
			...
			/*
			 * realm_test_util_slot_va_from_pa() return NULL if the address
...
TEST(slot_buffer, granule_map_buffer_unmap_TC2)
{
	...
	for (unsigned int i = 0U; i < NR_CPU_SLOTS; i++) {
		...
		/*
		 * Unmap the granules.
		 * For convenience, use the PA rather than the VA one.
		 */
		for (unsigned int j = 0U; j < MAX_CPUS; j++) {
			host_util_set_cpuid(j);
...
TEST(slot_buffer, ns_buffer_write_TC1)
{
	...
	for (unsigned int i = 0U; i < MAX_CPUS; i++) {
		 ...
		 * write data to it.
		 */
		block = (unsigned int)test_helpers_get_rand_in_range(0, 3);
...
TEST(slot_buffer, ns_buffer_write_TC2)
{
	 ...
	 */
	block = test_helpers_get_rand_in_range(0, (GRANULE_SIZE/sizeof(int)) - 1U);
...
IGNORE_TEST(slot_buffer, slot_buf_setup_xlat_TC1)
{
	/*
...
IGNORE_TEST(slot_buffer, slot_buf_init_TC1)
{
	/*
