 ...
 * excluding the last granule (in case we need extra room).
...
static inline uintptr_t get_rand_granule_addr(void) {
	...
	int random_granule = test_helpers_get_rand_in_range(0,
					test_helpers_get_nr_granules() - 2);
...
ASSERT_TEST(slot_buffer, granule_map_buffer_unmap_TC7)
{
	 ...
	 * TEST CASE 7:
	 *
	 * For a random CPU, try to map a granule address below to the
	 * start of the granules section to a random slot type other
	 * than SLOT_NS.
	 * The operation should generate an assertion failure.
	 ******************************************************************/
	...
	test_granule = (struct granule *)host_util_get_granule_base() - 1U;
	slot = (enum buffer_slot)test_helpers_get_rand_in_range(
						SLOT_NS + 1U, NR_CPU_SLOTS);
	cpuid = (unsigned int)test_helpers_get_rand_in_range(0, MAX_CPUS - 1);
	host_util_set_cpuid(cpuid);
