...
TEST(slot_buffer, buffer_map_unmap_internal_TC1)
{
	uintptr_t slot_va, expected_va, granule_addr;
	bool ns = false;
	bool expected_ns = false;

	/******************************************************************
	 * TEST CASE 1:
	 *
	 * For all possible slot buffer types, all possible CPUs and all
	 * possible security states, try to map a random granule.
	 * Then unmap it.
	 ******************************************************************/
	...
	for (unsigned int i = 0U; i < MAX_CPUS; i++) {
		...
		for (unsigned int j = 0U; j < NR_CPU_SLOTS; j++) {
			...
			for (unsigned int k = 0U; k < 2U; k++) {
				slot_va = (uintptr_t)buffer_map_internal(
							(enum buffer_slot)j,
							granule_addr,
							expected_ns);
				expected_va = slot_to_expected_va(
							(enum buffer_slot)j);

				/* Test the return value from granule_map() */
				POINTERS_EQUAL(expected_va, slot_va);

				/*
				 * Test that buffer_map_internal generated a
				 * page descriptor containing the right PA as
				 * well as the right security state.
				 * Use host_pa_to_slot_va(), as given a granule
				 * addr it will return the VA to which is
				 * mapped, so we can compare the result with
				 * the expected one.
				 */
				slot_va = host_pa_to_slot_va(granule_addr, &ns);
				POINTERS_EQUAL(expected_va, slot_va);
				CHECK_EQUAL(expected_ns, ns);

				/* Unmap the buffer */
				buffer_unmap_internal((void *)slot_va);

				/*
				 * host_pa_to_slot_va() return NULL if the
				 * address passed to it is not mapped to any
				 * slot buffer.
				 */
				slot_va = host_pa_to_slot_va(granule_addr,
							     NULL);
				POINTERS_EQUAL(NULL, slot_va);
...
TEST(slot_buffer, buffer_map_unmap_internal_TC2)
{
	uintptr_t slot_va, expected_va;
	 ...
	 * TEST CASE 2:
	 *
	 * For each possible slot buffer type and security state, map a
	 * different random granule to each one of the available CPUs.
	 * Then validate that the same PA is not mapped to two different
	 * CPUs.
	 ******************************************************************/
	...
	for (unsigned int i = 0U; i < NR_CPU_SLOTS; i++) {
		...
		for (unsigned int sec = 0U; sec < 2U; sec++) {

			/* Map a granule on each CPU for the same slot */
			for (unsigned int j = 0U; j < MAX_CPUS; j++) {
				host_set_cpuid(j);
				buffer_map_internal((enum buffer_slot)i,
						  granules_per_cpu[j],
						  expected_ns);
			}

			/*
			 * Iterate over all CPUs, ensuring that the granules
			 * are mapped into the slots for the right CPU.
			 */
			for (unsigned int j = 0U; j < MAX_CPUS; j++) {
				host_set_cpuid(j);
				for (unsigned int k = 0U; k < MAX_CPUS; k++) {
					slot_va = host_pa_to_slot_va(
						granules_per_cpu[k], NULL);

					/*
					 * Get the expected VA depending on the
					 * current CPU.
					 */
					expected_va = (j == k) ?
						slot_to_expected_va((enum buffer_slot)i) :
						(uintptr_t)NULL;
	...
	for (unsigned int i = 0U; i < MAX_CPUS; i++) {
		...
		for (unsigned int j = 0U; j < NR_CPU_SLOTS; j++) {
			...
			slot_va = (uintptr_t)granule_map(granule,
							 (enum buffer_slot)j);
			expected_va = slot_to_expected_va((enum buffer_slot)j);
			...
			POINTERS_EQUAL(expected_va,
				       host_pa_to_slot_va(slot_va, NULL));

			/* Unmap the buffer */
			buffer_unmap((void *)slot_va);

			/*
			 * host_pa_to_slot_va() return NULL if the address
			 * passed to it is not mapped to any slot buffer.
			 */
			POINTERS_EQUAL(NULL,
				       host_pa_to_slot_va(granule_addr, NULL));
...
{
	uintptr_t slot_va, expected_va, mapped_pa;
	struct granule *granule;
	uintptr_t granules_per_cpu[MAX_CPUS];
	...
	for (unsigned int i = 0U; i < NR_CPU_SLOTS; i++) {
		...
		for (unsigned int j = 0U; j < MAX_CPUS; j++) {
			 ...
			 */
			host_set_cpuid(j);
			mapped_pa = slot_to_arch_pa((enum buffer_slot)i, NULL);
...
TEST(slot_buffer, ns_buffer_write_TC1)
{
	...
	for (unsigned int i = 0U; i < MAX_CPUS; i++) {
		 ...
		 * write data to it.
		 */
		block = (unsigned int)get_rand_in_range(0, 3);
...
TEST(slot_buffer, ns_buffer_write_TC2)
{
	 ...
	 */
	do {
		cpu[0] = get_rand_in_range(0, MAX_CPUS - 1U);
		cpu[1] = get_rand_in_range(0, MAX_CPUS - 1U);
	} while (cpu[0] == cpu[1]);
	 ...
	 */
	block = get_rand_in_range(0, (GRANULE_SIZE/sizeof(int)) - 1U);
