 ...
 */
unsigned long s2tte_create_assigned_ns_with_params(unsigned long s2tte,
						   unsigned long pa,
						   long level)
{
	assert(addr_is_level_aligned(pa, level));
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_ns(unsigned long *s2tt, unsigned long parent_s2tte, unsigned long pa, long level)
{
	const unsigned long map_size = s2tte_map_size(level);

	for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
		s2tt[i] = s2tte_create_assigned_ns_with_params(parent_s2tte, pa, level);
		pa += map_size;
	}
	dsb(ish);
}
