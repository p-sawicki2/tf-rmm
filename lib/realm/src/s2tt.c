 ...
 *
 * ======================================================================================
 * s2tte type           level DESC_TYPE[1:0] HIPAS[4:2]    RIPAS[6:5]   NS  ptr.alignment
 * ======================================================================================
 ...
 * by the host.
 */
bool host_ns_s2tte_is_valid_lpa2(unsigned long s2tte, long level)
{
	unsigned long mask = table_entry_to_phys_lpa2(~0UL, level) |
			     S2TTE_MEMATTR_MASK |
			     S2TTE_AP_MASK |
			     S2TTE_ENTRY_MSB_MASK;
...
bool host_ns_s2tte_is_valid_non_lpa2(unsigned long s2tte, long level)
{
...
{
	unsigned long mask = table_entry_to_phys(~0UL, level) |
			     S2TTE_MEMATTR_MASK |
			     S2TTE_AP_MASK |
			     /*
			      * When FEAT_LPA2 is enabled, the two MSB bits
			      * of the OA are stored on the SH field for a
			      * non-LPA2 TTE, so include the SH field anyway.
			      */
			     S2TTE_SH_MASK;
...
enum ripas s2tte_get_ripas(unsigned long s2tte)
{
	...
	case S2TTE_INVALID_RIPAS_RAM:
		return RIPAS_RAM;
	case S2TTE_INVALID_RIPAS_DESTROYED:
...
void s2tt_init_assigned_ns(unsigned long *s2tt, unsigned long pa, long level)
{
	...
	for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
		s2tt[i] = s2tte_create_assigned_ns(create_empty_tte(pa), level);
		pa += map_size;
	}
	dsb(ish);
}
