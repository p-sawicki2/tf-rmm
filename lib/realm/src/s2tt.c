 ...
 * Depending on whether FEAT_LPA2 is supported, this number might be different.
 */
...
#define NR_RTT_LEVELS		(RTT_PAGE_LEVEL - MIN_STARTING_LEVEL + 1U)
#define NR_RTT_LEVELS_LPA2	(RTT_PAGE_LEVEL - MIN_STARTING_LEVEL_LPA2 + 1U)
 ...
 */
void invalidate_page(const struct realm_s2_context *s2_ctx,
		     unsigned long addr)
{
...
static unsigned long s2_addr_to_idx(unsigned long addr, long level)
{
	...
	addr &= ((level == MIN_STARTING_LEVEL_LPA2) ?
			(1UL << 4U) - 1UL :
...
{
	unsigned long pa = addr_level_mask(entry, RTT_PAGE_LEVEL);

	/*
	 * The two MSBs of the OA is not contiguous to the rest of the
	 * OA within an entry when FEAT_LP2A is present.
	 */
			  ...
			  struct rtt_walk *wi)
{
	...
	int i, last_level;
	int min_starting_level = (is_feat_lpa2_4k_present() == true) ?
...
unsigned long s2tte_pa(unsigned long s2tte, long level)
{
	...
	return (is_feat_lpa2_4k_present() == true) ?
		(pa & ~MASK(S2TTE_OA_MSB)) |
			INPLACE(S2TTE_OA_MSB, EXTRACT(S2TTE_ENTRY_MSB, s2tte))
		: pa;
}
