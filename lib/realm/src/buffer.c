 ...
 */
static struct slot_tbl_info tbl_info_cache[MAX_CPUS];
...
struct slot_tbl_info *get_cached_tbl_info(void)
{
...
{
	uint64_t *entry = xlat_get_tte(get_cached_tbl_info(),
				       slot_to_va(slot));
...
void *buffer_map_internal(enum buffer_slot slot, unsigned long addr)
{
	...
	struct slot_tbl_info *entry = get_cached_tbl_info();
