...
#define E_RMM_BOOT_SUCCESS				UL(0)
#define E_RMM_BOOT_UNKNOWN_ERROR			UL(-1)
#define E_RMM_BOOT_VERSION_NOT_VALID			UL(-2)
#define E_RMM_BOOT_CPUS_OUT_OF_RANGE			UL(-3)
#define E_RMM_BOOT_CPU_ID_OUT_OF_RANGE			UL(-4)
#define E_RMM_BOOT_INVALID_SHARED_BUFFER		UL(-5)
#define E_RMM_BOOT_MANIFEST_VERSION_NOT_SUPPORTED	UL(-6)
#define E_RMM_BOOT_MANIFEST_DATA_ERROR			UL(-7)
 ...
 * from Boot manifest v0.2 onwards.
 *
 * Args:
 *	- max_num_banks:	Maximum number of platform's DRAM banks
 *				supported.
 *	- plat_dram:		The address of the pointer to the platform
 *				DRAM info structure setup by EL3 Firmware,
 *	  			or NULL in case of DRAM data structure errors.
 *
 * Return:
 *	- E_RMM_BOOT_SUCCESS	On success or an error code otherwise.
 */
unsigned long rmm_el3_ifc_get_dram_data_validated_pa(
				unsigned long max_num_banks,
				struct dram_info **plat_dram_info);
 ...
 */
__dead2 void rmm_el3_ifc_report_fail_to_el3(uint64_t ec);
