...
struct xlat_ctx_cfg {
	 ...
	 * The list is terminated by the first entry with size == 0 or when
	 * all entries are used (as specified by PLAT_CMN_MAX_MMAP_REGIONS).
	 */
	struct xlat_mmap_region mmap[PLAT_CMN_MAX_MMAP_REGIONS];
 ...
 *	- mm: NULL terminated list of memory map regions to add to the
 ...
 * and set of translation tables overwriting any previous context state
 * (except for the xlat_ctx_cfg structure which is not modified).
 * This function initializes the associated xlat_ctx_tbls structure with
 * the set of tables passed destroying any previous configuration that might
 * been there before and creates the translation tables for the context.
 *
 * Arguments:
 ...
 *	 xlat_ctx_cfg structure, and it just adds it to the context.
 *	 If the xlat_ctx_cfg is not properly initialized, the creation
 *	 of the translation tables will fail, and the function will return
...
/*
 * Return true if the xlat_ctx is fully initialized.
 */
