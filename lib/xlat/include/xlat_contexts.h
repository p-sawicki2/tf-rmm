...
/*
 * Initializes the translation context (xlat_ctx) if not already initialized
 * with the given xlat_ctx_cfg. The tables are created according to the
 * configuration in the provided memory and the associated xlat_ctx_tbls
 * structure is initialized. Must be called before the MMU is enabled.
 *
 * Arguments:
