...
#define STACK_BUFFER_SIZE	(RMM_CPU_STACK_SIZE * MAX_CPUS)
/* Calculate the slot buffer's virtual address */
#define SLOT_VIRT (((UL(~0)) - (XLAT_HIGH_VA_SLOT_NUM * GRANULE_SIZE)) + 1U)

#define RMM_CPU_STACK_BOTTOM_VA (SLOT_VIRT - CPU_STACK_GAP)
#define CPU_STACK_VIRT (RMM_CPU_STACK_BOTTOM_VA - RMM_CPU_STACK_SIZE)
