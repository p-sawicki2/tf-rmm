...
#define CPU_STACK_SIZE		(XLAT_NUM_PAGES_PER_STACK * PAGE_SIZE)
/* Leave some pages of gap above the stack top */
#define GAP_PAGE_COUNT		(U(1))
#define CPU_STACK_GAP		(GAP_PAGE_COUNT * PAGE_SIZE)
#define STACK_BUFFER_SIZE	(CPU_STACK_SIZE * MAX_CPUS)
/* Calculate the slot buffer's virtual address */
#define SLOT_VIRT (((UL(~0)) - (XLAT_HIGH_VA_SLOT_NUM * GRANULE_SIZE)) + 1)

#define CPU_STACK_BOTTOM_VIRT (SLOT_VIRT - CPU_STACK_GAP)
#define CPU_STACK_VIRT (CPU_STACK_BOTTOM_VIRT - CPU_STACK_SIZE)
