...
/*
 * Minimum table level supported by the architecture. We take the case for
 * FEAT_LPA2 being enabled as this is a special case that needs special
 * consideration in the library internals. Also the library is in charge
 * of calculating the minimum level when creating the translation tables
 * so there is no need to explicitly check the level in any case other
 * than for FEAT_LPA2.
 */
