 ...
 * The value of TCR.TxSZ must be in the range 12 to 48 [1], which
 * means that the virtual address space width must be in the range 52
...
/* Minimum value of TCR_ELx.T(0,1)SZ is 12, for a VA of 52 bits */
...
/* Macro to extract the OA from a given TTE */
#define GET_TTE_OA(_tte)						\
	((uint64_t *)((BIT_MASK_ULL(TABLE_ADDR_MSB, TABLE_ADDR_SHIFT)	\
	& (_tte)) | INPLACE(OA_MSB, EXTRACT(TTE_OA_MSB, (_tte)))))
