 ...
 *	  an entry at the beginning of a table of level 'first_lvl'.
 *	- Second region mapped at a random index of a table whose final
 *	  lookup level is 'last_lvl'. This region will be descendant of
 *	  an entry at a random index of a table of level 'first_lvl'.
 *	- Third region mapped at the end of a table whose final
 *	  lookup level is 'last_lvl'. This region will be descendant of
 *	  an entry at the end of a table of level 'first_lvl'.
 *
 * The base address of the mmap regions can be increased by the
 * specified 'offset' so the last level table (or any of its parents)
 * will be mapped to a tte entry > 0 on the parent table. Therefore, 'offset'
 * must be aligned with the size of 'last_lvl' block.
 *
				   ...
				   bool allow_transient)
{
	...
	assert(levels != NULL);
	assert(ALIGNED(offset, (1ULL << XLAT_ADDR_SHIFT(last_lvl))));
	...
	*granularity = 1ULL << XLAT_ADDR_SHIFT(last_lvl);

	mmap_start_va += offset;

	for (unsigned i = 0U; i < 3U; i++) {
		mmap[i].base_va = mmap_start_va;
		if ((first_lvl + 1U) < last_lvl)
		{
			/*
			 * Add an offset to the base VA so that this region
			 * is mapped at the same index used for the higher
			 * level but on the first level.
			 */
			mmap[i].base_va += tbl_idxs[i] *
				(1ULL << XLAT_ADDR_SHIFT(first_lvl + 1U));
		}
				 ...
				 unsigned int *expected_levels)
{
	...
	for (unsigned int i = 0U; i < ctx->cfg->mmap_regions; i++) {
		...
		CHECK_EQUAL((tte & XLAT_TESTS_TTE_ADDR_MASK), (pa & ~pa_mask));
...
TEST(xlat_tests_G2, xlat_ctx_init_TC6)
{
	 ...
	 *	- First region mapped at the beginning of a table whose final
	 *	  lookup level is 'last_lvl'. This region will be descendant of
	 *	  an entry at the beginning of a table of level 'base_lvl'.
	 *	- Second region mapped at a random index of a table whose final
	 *	  lookup level is 'last_lvl'. This region will be descendant of
	 *	  an entry at a random index of a table of level 'base_lvl'.
	 *	- Third region mapped at the end of a table whose final
	 *	  lookup level is 'last_lvl'. This region will be descendant of
	 *	  an entry at the end of a table of level 'base_lvl'.
	 *
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		for (end_lvl = 1U;
			...
			for (base_lvl = 0U;
				...
				/*
				 * Get a random test address from a random mmap
				 * region. This needs to be a valid address
				 * within a table whose base VA is not the context
				 * base VA.
				 */
				mmap_idx = test_helpers_get_rand_in_range(0, 2);
...
TEST(xlat_tests_G2, xlat_get_llt_from_va_TC2)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		for (unsigned int j = 0; j < (i + 1U); j++) {
			...
			start_va = gen_va_space_params_by_lvl(2UL, va_region,
							      &va_size);
			 ...
			 *
			 * Add an offset of L2 block size, so the last level
			 * table does not start at the beginning of the
			 * VA space.
			 */
			offset = XLAT_BLOCK_SIZE(2U);
			retval = gen_mmap_array_by_level(&init_mmap[0U],
							&tbl_idx[0U],
							&expected_lvls[0U],
							3U, 3U, 3U,
...
TEST(xlat_tests_G2, xlat_get_llt_from_va_TC3)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/* VA space boundaries */
		start_va = gen_va_space_params_by_lvl(0UL, va_region,
							      &va_size);
...
ASSERT_TEST(xlat_tests_G2, xlat_get_llt_from_va_TC4)
{
	...
	uint64_t start_va, end_va, test_va;
	xlat_addr_region_id_t va_region;
...
ASSERT_TEST(xlat_tests_G2, xlat_get_llt_from_va_TC8)
{
	...
	retval = xlat_ctx_cfg_init(&cfg, va_region, &init_mmap, 1U,
				   MAX_VIRT_ADDR_SPACE_SIZE);
...
TEST(xlat_tests_G2, xlat_get_tte_ptr_TC2)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 *
	 * Initialize a translation context with a given VA space and
	 * random mmap regions. Then try to get a tte using
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		start_va = gen_va_space_params_by_lvl(0U, va_region, &va_size);
		 ...
		 * 'start_va'. This way, we can pick up an address below the
		 * minimum VA mapped by the last level table to test the low
		...
		retval = xlat_ctx_cfg_init(&cfg, va_region, &init_mmap[0], 3U,
					MAX_VIRT_ADDR_SPACE_SIZE);
...
TEST(xlat_tests_G2, xlat_unmap_memory_page_TC1)
{
	...
	for (end_lvl = 1U; end_lvl <= XLAT_TABLE_LEVEL_MAX; end_lvl++) {
		for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
			...
			start_va = gen_va_space_params_by_lvl(0U, va_region,
							      &va_size);
...
TEST(xlat_tests_G2, xlat_unmap_memory_page_TC2)
{
	...
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	uint64_t start_va, test_va, offset;
	size_t va_size, granularity;
	unsigned int mmap_count;
	unsigned int tte_idx, tte_lvl;
	xlat_addr_region_id_t va_region;
		...
		for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
			...
			for (unsigned j = 0U; j < mmap_count; j++) {
				...
				pa += test_helpers_get_rand_in_range(1,
						PAGE_SIZE - 1);
				val_tte |= pa & XLAT_TESTS_TTE_ADDR_MASK;
...
TEST(xlat_tests_G2, xlat_map_memory_page_with_attrs_TC2)
{
	...
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	uint64_t start_va, test_va, test_pa, offset;
	size_t va_size, granularity;
	unsigned int mmap_count;
	unsigned int tte_idx, tte_lvl;
	xlat_addr_region_id_t va_region;
