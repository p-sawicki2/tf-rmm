 ...
 * shuffle.
 */
void xlat_tests_base::buffer_shuffle(unsigned char *buf, size_t size,
				     unsigned int stride)
{
 ...
 */
void xlat_tests_base::xlat_test_cfg_init_setup(struct xlat_ctx_cfg *cfg,
 ...
 */
uint64_t xlat_tests_base::gen_va_space_params_by_lvl(int level,
					xlat_addr_region_id_t region,
					size_t *va_size)
{
 ...
 */
int xlat_tests_base::gen_mmap_array_by_level(xlat_mmap_region *mmap,
 ...
 */
void xlat_tests_base::validate_xlat_tables(xlat_ctx *ctx,
...
void xlat_tests_base::xlat_get_llt_from_va_prepare_assertion(
						struct xlat_ctx *ctx,
...
void xlat_tests_base::validate_ttbrx_el2(struct xlat_ctx *ctx)
{
...
void xlat_tests_base::validate_tcr_el2(struct xlat_ctx *low_ctx,
...
void xlat_tests_base::setup(bool lpa2) {
	test_helpers_init();
	xlat_test_hepers_arch_init(lpa2);
}

void xlat_tests_base::map_region_full_spec_tc1(void)
{
	/*************************************************************
	 * TEST CASE 1:
	 *
	 * Instantiate a struct xlat_mmap_region structure and
	 * populate it using MAP_REGION_FULL_SPEC macro. Verify that
	 * the structure fields are right.
	 *************************************************************/
...
void xlat_tests_base::map_region_tc1(void)
{
	/**************************************************************
	 * TEST CASE 1:
	 *
...
void xlat_tests_base::map_region_flat_tc1(void)
{
	/**************************************************************
	 * TEST CASE 1:
	 *
	 ...
	 */
	struct xlat_mmap_region test_mmap = MAP_REGION_FLAT(base_addr,
							validation_mmap.size,
							validation_mmap.attr
	);
...
void xlat_tests_base::map_region_transient_tc1(void)
{
	/**************************************************************
	 * TEST CASE 1:
	 *
...
void xlat_tests_base::xlat_ctx_cfg_init_tc1(void)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Initialize the test structures with
		 * the expected values.
		 */
		...
		retval = xlat_ctx_cfg_init(&test_cfg, region,
					   &init_mmap[0],
					   XLAT_TESTS_MAX_MMAPS,
					   max_va_size);
...
void xlat_tests_base::xlat_ctx_cfg_init_tc2(void)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		 ...
		 * the expected values.
		 */
		xlat_test_cfg_init_setup(&foo_cfg, &init_mmap[0],
					 &val_mmap[0],
					 XLAT_TESTS_MAX_MMAPS,
					 max_va_size,
					 region);
...
void xlat_tests_base::xlat_ctx_cfg_init_tc3(void)
{
...
void xlat_tests_base::xlat_ctx_cfg_init_tc4(void)
{
...
void xlat_tests_base::xlat_ctx_cfg_init_tc5(void)
{
	struct xlat_ctx_cfg test_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	xlat_addr_region_id_t region;
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/* Clean the data structures */
		memset((void *)&test_cfg, 0, sizeof(struct xlat_ctx_cfg));
		memset((void *)&init_mmap[0], 0, sizeof(xlat_mmap_region) *
							XLAT_TESTS_MAX_MMAPS);

		/* Initialize the test structure */
		retval = xlat_ctx_cfg_init(&test_cfg, region, &init_mmap[0],
					   0U, XLAT_TEST_MAX_VA_SIZE());
...
void xlat_tests_base::xlat_ctx_cfg_init_tc6(void)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Get a VA large enough for testing but leaving
		 * some room for it to grow without being larger than
		 * the maximum allowed.
		 */
		...
		/* Initialize the test structure */
		retval = xlat_ctx_cfg_init(&test_cfg, region, &init_mmap[0],
					   XLAT_TESTS_MAX_MMAPS, test_va_size);
		...
		/*
		 * Test with a VA Size lower than
...
void xlat_tests_base::xlat_ctx_cfg_init_tc7(void)
{
	struct xlat_ctx_cfg test_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	struct xlat_mmap_region val_mmap[XLAT_TESTS_MAX_MMAPS];
	 ...
	 * TEST CASE 7:
	 *
	 * Try to initialize an already initialized
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		 ...
		 */
		xlat_test_cfg_init_setup(&test_cfg, &init_mmap[0],
...
void xlat_tests_base::xlat_ctx_cfg_init_tc8(void)
{
	struct xlat_ctx_cfg test_cfg, foo_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	struct xlat_mmap_region val_mmap[XLAT_TESTS_MAX_MMAPS];
	...
	unsigned int pa_range_bits_arr[] = {
		PARANGE_0000_WIDTH, PARANGE_0001_WIDTH,
		PARANGE_0010_WIDTH, PARANGE_0011_WIDTH,
		PARANGE_0100_WIDTH, PARANGE_0101_WIDTH,
		PARANGE_0110_WIDTH
	};
	uint64_t max_va_size =	XLAT_TEST_MAX_VA_SIZE();
	 ...
	 *	- 'base_pa' < maximum supported PA && 'base_pa' +
	 *	  'size' > maximum supported PA
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		 ...
		 * such as 'base_pa' + 'size' is above the maximum PA
		 * but 'base_va' + 'size' is within the valid range.
		 *
		 * Note that 'size' will be at least the size of a
		 * memory page.
		 */
		init_mmap[XLAT_TESTS_MAX_MMAPS - 1U].base_pa =
				(1ULL << pa_range_bits_arr[index]) - PAGE_SIZE;
		...
		/*
		 * Overwrite the mmap structures to make the PAs
		 * overlap for the next test
		 */
		mmap_index = (unsigned int)test_helpers_get_rand_in_range(1,
						XLAT_TESTS_MAX_MMAPS - 1);
		/*
		 * The base_pa of mmap entry at 'mmap_index' is
		 * adjusted to an offset of 'PAGE_SIZE' of previous
		 * entry ('base_pa' is set to the same value as
		 * 'base_va'). Each entry has a size of at least 2
		 * pages, so the PA of the regions will overlap.
		 */
		...
		/*
		 * Overwrite the PA on one of the memory map
		 * regions to make it misaligned.
		 */
...
void xlat_tests_base::xlat_ctx_cfg_init_tc9(void)
{
	struct xlat_ctx_cfg test_cfg, foo_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	struct xlat_mmap_region val_mmap[XLAT_TESTS_MAX_MMAPS];
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		 ...
		 */
		xlat_test_cfg_init_setup(&foo_cfg, &init_mmap[0], &val_mmap[0],
					 XLAT_TESTS_MAX_MMAPS, max_va_size, region);
		...
		/*
		 * Overwrite the VA on one of the memory map
		 * regions to make it misaligned.
		 */
		mmap_index = (unsigned int)test_helpers_get_rand_in_range(0,
						XLAT_TESTS_MAX_MMAPS - 1);
		init_mmap[mmap_index].base_va +=
				test_helpers_get_rand_in_range(1, PAGE_SIZE - 1);
		...
		/*
		 * Overwrite the mmap structures to
		 * make the VAs overlap.
		 */
		mmap_index = (unsigned int)test_helpers_get_rand_in_range(1,
						XLAT_TESTS_MAX_MMAPS - 1);
		/*
		 * The base_va of mmap entry at 'mmap_index' is
		 * adjusted to an offset of 'PAGE_SIZE' of previous
		 * entry. Each entry has a size of at least 2 pages,
		 * so the regions will overlap whilst keeping in
		 * ascending order of VA.
		 */
		init_mmap[mmap_index].base_va =
				init_mmap[mmap_index - 1U].base_va + PAGE_SIZE;
...
void xlat_tests_base::xlat_ctx_cfg_init_tc10(void)
{
	struct xlat_ctx_cfg test_cfg, foo_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	struct xlat_mmap_region val_mmap[XLAT_TESTS_MAX_MMAPS];
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Overwrite the size on one of the memory map
		 * regions to make it misaligned.
		 */
		mmap_index = (unsigned int)test_helpers_get_rand_in_range(0,
					XLAT_TESTS_MAX_MMAPS - 1);
...
void xlat_tests_base::xlat_ctx_cfg_init_tc11(void)
{
	struct xlat_ctx_cfg test_cfg, foo_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	struct xlat_mmap_region val_mmap[XLAT_TESTS_MAX_MMAPS];
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Overwrite a memory mapping region to make
		 * it a duplicate of the previous one.
		 */
		mmap_index = (unsigned int)test_helpers_get_rand_in_range(1,
							XLAT_TESTS_MAX_MMAPS - 1);
...
void xlat_tests_base::xlat_ctx_cfg_init_tc12(void)
{
	struct xlat_ctx_cfg test_cfg, foo_cfg;
	struct xlat_mmap_region init_mmap[XLAT_TESTS_MAX_MMAPS];
	struct xlat_mmap_region val_mmap[XLAT_TESTS_MAX_MMAPS];
	 ...
	 * TEST CASE 12:
	 *
	 * Try to initialize a xlat_ctx_cfg structure with out of
	 * order memory mapping areas.
	 *************************************************************/
...
void xlat_tests_base::xlat_ctx_init_tc1(void)
{
	struct xlat_ctx ctx;
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Validate that the configuration has
		 * not been altered.
		 */
		 ...
		 *
		 * NOTE: As the memory regions are random,
		 * the `next_table` field is not validated here.
		 * Instead, it will be validated for each especific
		 * test later.
		 */
...
void xlat_tests_base::xlat_ctx_init_tc2(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_ctx_init_tc3(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Test xlat_ctx_init() with a NULL
...
void xlat_tests_base::xlat_ctx_init_tc4(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_ctx_init_tc5(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * TEST CASE 5:
	 *
	 * Try to initialize a context with valid random memory
	 * mapping areas and the MMU enabled.
	 *************************************************************/
...
void xlat_tests_base::xlat_ctx_init_tc6(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * ranging from level 1 (lowest level at which a valid walk
	 * can finish) to XLAT_TABLE_LEVEL_MAX.
	 *
	 * For each possible (va_region, base_lvl, end_lvl) triplet
	 * for abase table there will be three mmap regions created:
	 *
	 *	- First region mapped at the beginning of a table whose
	 *	  final lookup level is 'last_lvl'. This region will be
	 *	  descendant of an entry at the beginning of a
	 *	  'base_lvl' table.
	 *	- Second region mapped at a random index of a table
	 *	  whose final lookup level is 'last_lvl'. This region
	 *	  will be descendant of an entry at a random index of
	 *	  a 'base_lvl' table.
	 *	- Third region mapped at the end of a table whose final
	 *	  lookup level is 'last_lvl'. This region will be
	 *	  descendant of an entry at the end of a 'base_lvl'.
	 *
	 * Then verify that the tables can be walked and that the
	 * levels, offsets and attributes on the ttes are as expected.
	 *
	...
	for (end_lvl = 1U; end_lvl <= XLAT_TABLE_LEVEL_MAX; end_lvl++) {
		for (int i = 0U; i < VA_REGIONS; i++) {
			...
			for (base_lvl = XLAT_TEST_MIN_LVL();
				...
				validate_xlat_tables(&ctx, &tbl_idx[0U], end_lvl);
			}
		}
	}
}

void xlat_tests_base::xlat_get_llt_from_va_tc1(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_get_llt_from_va_tc2(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	...
	for (int i = 0U; i < VA_REGIONS; i++) {
		...
		for (unsigned int j = 0; j < (i + 1U); j++) {
			...
			if (j == 0U) {
				 ...
				 * The upper range of the address
				 * is arbitrary.
				 */
...
void xlat_tests_base::xlat_get_llt_from_va_tc3(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * TEST CASE 3:
	 *
	 * Test xlat_get_llt_from_va() with an unmapped VA belonging
	 * to the context VA space.
	 *************************************************************/
	...
	for (int i = 0U; i < VA_REGIONS; i++) {
		...
		/* Check the return value */
		CHECK_VERBOSE((retval == 0), "Testing VA 0x%lx", test_va);
		VERBOSE("\n");
	}
}

void xlat_tests_base::xlat_get_llt_from_va_tc4(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_get_llt_from_va_tc5(void)
{
	struct xlat_llt_info tbl_info;
...
void xlat_tests_base::xlat_get_llt_from_va_tc6(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	...
	xlat_get_llt_from_va_prepare_assertion(&ctx, &cfg,
					       &tbls, &init_mmap);
	memset((void *)&tbl_info, 0, sizeof(struct xlat_llt_info));
...
void xlat_tests_base::xlat_get_llt_from_va_tc7(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_get_llt_from_va_tc8(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_get_llt_from_va_tc9(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_get_tte_ptr_tc1(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 *	1. It tries to get the TTE via xlat_get_tte() for a
	 *	   lower VA  than the base VA.
	 *	2. It tries to get the TTE for a higher VA than is
	 *	   mapped by the last level table.
	 *************************************************************/
	...
	for (int i = 0U; i < VA_REGIONS; i++) {
		...
		for (unsigned int i = 0U; i < 3U; i++) {
			...
			/*
			 * Perform a table walk to get the table
			 * containing the tte we are insterested in as
			 * well as the index of that tte in the table.
			 */
			retval = xlat_test_helpers_table_walk(&ctx, test_va,
							       &tte, &table,
							       &level, &index);
		 ...
		 * VA mapped by 'tbl_info'. Use init_mmap[1] for this
		 * test.
		 */
		 ...
		 * VA mapped by 'tbl_info'. Use init_mmap[0] for this
		 * test.
		 */
		...
		test_va = tbl_info.llt_base_va +
					XLAT_BLOCK_SIZE(tbl_info.level - 1);
		test_va += test_helpers_get_rand_in_range(1, PAGE_SIZE - 1);
...
void xlat_tests_base::xlat_get_tte_ptr_tc2(void)
{
	/*************************************************************
	 * TEST CASE 2:
	 *
...
void xlat_tests_base::xlat_get_tte_ptr_tc3(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * TEST CASE 3:
	 *
	 * Try to get a tte using xlat_get_tte() in which 'level'
	 * is below the minimum for the current architecture
	 * implementation.
	 *************************************************************/
...
void xlat_tests_base::xlat_get_tte_ptr_tc4(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * above the maximum for the current architecture
	 * implementation.
	 ***************************************************************/
...
void xlat_tests_base::xlat_unmap_memory_page_tc1(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 *
	 * For each possible (va_region, end_lvl) tuple, there will
	 * be three mmap regions created:
	 *
	 *	- First region mapped at the beginning of a table
	 *	  whose final lookup level is 'end_lvl'
	 *	- Second region mapped at a random tte of a table
	 *	  whose final lookup level is 'end_lvl'
	...
	for (end_lvl = 1; end_lvl <= XLAT_TABLE_LEVEL_MAX; end_lvl++) {
		for (int i = 0U; i < VA_REGIONS; i++) {
			...
			for (unsigned int j = 0U; j < mmap_count; j++) {
				...
				/*
				 * Perform a table walk to retrieve the
				 * table where the VA is mapped along
				 * with the index of the TTE within the
				 * table.
				 */
				retval = xlat_test_helpers_table_walk(&ctx,
							test_va, &tte,
				...
				/*
				 * Retrieve the xlat_llt_info
				 * structure needed to feed
...
void xlat_tests_base::xlat_unmap_memory_page_tc2(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 *
	 *	- Try addresses below and above the range mapped by
	 *	  the xlat_llt_info structure on a transient-valid
	 *	  entry.
	...
	for (int i = 0U; i < VA_REGIONS; i++) {
		...
		/*
		 * We generate the mmap regions to use. We will
		 * be interested in init_mmap[1].
		 */
		retval = gen_mmap_array_by_level(&init_mmap[0U],
						 &tbl_idx[0U],
						 mmap_count,
		...
		/*
		 * Make the TTEs of the mapped region, which is
		 * expected to be valid, transient valid.
		 */
		test_va = init_mmap[1U].base_va + ctx.cfg->base_va;

		/*
		 * Perform a table walk to retrieve the table
		 * where the VA is mapped along with the index of
		 * the TTE within the table.
		 */
		retval = xlat_test_helpers_table_walk(&ctx, test_va,
						      &tte,
						      &tbl_ptr,
		 ...
		 * below the start of init_mmap[0U]. This gives us an
		 * address below the range mapped by table we retrieved.
		 */
		...
		/*
		 * Repeat the process, this time with an address on
		 * a page after the one mapped by init_mmap[2U].
		 * This gives us an address over the range mapped by
		 * table we retrieved.
		 */
		...
		/* Verify that the return is as expected */
		CHECK_VERBOSE((retval == -EFAULT),
		      "Testing VA 0x%lx on a ninvalid TTE", test_va);
...
void xlat_tests_base::xlat_unmap_memory_page_tc3(void)
{
	/*************************************************************
	 * TEST CASE 3:
	 *
...
void xlat_tests_base::xlat_map_memory_page_with_attrs_tc1(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	...
	for (end_lvl = 1; end_lvl <= XLAT_TABLE_LEVEL_MAX; end_lvl++) {
		for (int i = 0U; i < VA_REGIONS; i++) {
			...
			/*
			 * For each one of the mmap regions:
			 * - Generate a random VA within the mmap VA
			 *   space.
			 * - generate a set of random attributes.
			 * - Map a random PA to the generated VA and
			 *   with the generated attributes.
			 * - call xlat_unmap_memory_page_map_with_attrs()
			...
			for (unsigned int j = 0U; j < mmap_count; j++) {
				...
				/*
				 * Generate the validation TTE.
				 * For convenience, create an
				 * identity mapping.
				 */
				...
				/*
				 * Add an arbitrary offset to PA to be
				 * passed to xlat_map_memory_page_with_attrs()
				 */
				...
				val_tte |= (1ULL << TRANSIENT_FLAG_SHIFT);
				...
				/*
				 * Retrieve the xlat_llt_info structure
				 * needed to feed
				...
				/*
				 * Try to map the PA with the attributes
				 * to the `test_va`
				 */
...
void xlat_tests_base::xlat_map_memory_page_with_attrs_tc2(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	...
	for (int i = 0U; i < VA_REGIONS; i++) {
		...
		/*
		 * We generate the mmap regions to use. We will be
		 * interested in init_mmap[1] for the transient-invalid
		 * tests and in init_mmap[2] for the rest of tests.
		 */
		retval = gen_mmap_array_by_level(&init_mmap[0U],
						 &tbl_idx[0U],
						 mmap_count, base_lvl,
		...
		/*
		 * Test xlat_map_memory_page_with_attrs() with a
		 * valid address within init_mmap[0]. This gives us an
		 * address below the range mapped by table we retrieved
		 * (which belongs to init_mmap[1]). For simplicity, set
		 * the attributes and the PA both to 0x0.
		 */
		test_va = init_mmap[0U].base_va + ctx.cfg->base_va;
		test_va += test_helpers_get_rand_in_range(0,
						init_mmap[0U].size - 1);
		...
		retval = xlat_map_memory_page_with_attrs(&tbl_info,
							 test_va,
							 0ULL, 0ULL);
		...
		/*
		 * Repeat the process, this time with an address on a
		 * page mapped by init_mmap[2]. This gives us an
		...
		/*
		 * Perform a table walk to retrieve the table where the
		 * VA is mapped along with the index of the TTE within
		 * the table.
		 */
		retval = xlat_test_helpers_table_walk(&ctx, test_va,
						      &tte, &tbl_ptr,
		...
		/*
		 * Take a snapshot of the TTE. This will be used to
		 * verify that the TTE hasn't been altered.
		 */
		val_tte = tbl_ptr[tte_idx];
		...
		/*
		 * Take a snapshot of the TTE. This will be used
		 * to verify that the TTE hasn't been altered.
		 */
		val_tte = tbl_ptr[tte_idx];

		/*
		 * Now try to map a valid VA. In this case the
		 * associated TTE will contain a transient valid
		 * mapping.
		 */
		test_va = init_mmap[2U].base_va + ctx.cfg->base_va;
		test_va += test_helpers_get_rand_in_range(0, PAGE_SIZE - 1);
...
void xlat_tests_base::xlat_map_memory_page_with_attrs_tc3(void)
{
	/*************************************************************
	 * TEST CASE 3:
	 *
...
void xlat_tests_base::xlat_arch_setup_mmu_cfg_tc1(void)
{
	 ...
	 * TEST CASE 1:
	 *
	 * Generate a translation context for each region and
	 * configure the MMU registers based on both contexts. Verify
	 * that the right parameters have been configured.
	 *************************************************************/
	...
	for (int i = 0U; i < VA_REGIONS; i++) {
		...
		xlat_test_helpers_rand_mmap_array(&init_mmap[i], 1U,
						  start_va, end_va);

		retval = xlat_ctx_cfg_init(&cfg[i], va_region,
					   &init_mmap[i],
					   1U, max_va_size);
		CHECK_TRUE(retval == 0);
...
void xlat_tests_base::xlat_arch_setup_mmu_cfg_tc2(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * TEST CASE 2:
	 *
	 * Generate a valid translation context for one of the
	 * regions and overwrite it to test different failure
	 * conditions on xlat_arch_setup_mmu_cfg():
	 *
	...
	/* Generate only a single mmap region for each region */
	xlat_test_helpers_rand_mmap_array(&init_mmap, 1U,
					  start_va, end_va);
	...
	/*
	 * Force the architecture to report 4K granularity
	 * as not available.
	 */
	...
	/*
	 * Force the architecture to report 4K granularity available
	 * without support for LPA2 but with a PA size of more than 48
	 * bits. Note that this scenario should never happen on the
	 * architecture however, the library still checks for this.
	 */
	host_write_sysreg("id_aa64mmfr0_el1",
...
void xlat_tests_base::xlat_arch_setup_mmu_cfg_tc3(void)
{
	/*************************************************************
	 * TEST CASE 3:
	 *
...
void xlat_tests_base::xlat_arch_setup_mmu_cfg_tc4(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
	 ...
	 * TEST CASE 4:
	 *
	 * Test xlat_arch_setup_mmu_cfg() with a context in which
...
void xlat_tests_base::xlat_arch_setup_mmu_cfg_tc5(void)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
void xlat_tests_base::xlat_get_oa_from_tte_tc1(void)
{
	 ...
	 * TEST CASE 1:
	 *
	 * Test xlat_get_oa_from_tte() with 4K granularity and with
	 * and without LPA2 support.
	 *************************************************************/

	/*
	 * Generate a random TTE to test. We are not concerned about
	 * the validity of the TTE or about any bitfield that is not
	 * part of the output address, as xlat_get_oa_from_tte() is
	 * expected to just ignore those.
	 */
	test_tte = ~0ULL;
	...
	val_addr |= INPLACE(OA_BITS_50_51, EXTRACT(TTE_OA_BITS_50_51,
						   test_tte));
	...
	/*
	 * Repeat the test, by disabling support for
	 * FEAT_LPA2 this time.
	 */
	host_write_sysreg("id_aa64mmfr0_el1",
		INPLACE(ID_AA64MMFR0_EL1_PARANGE, 5U) |
