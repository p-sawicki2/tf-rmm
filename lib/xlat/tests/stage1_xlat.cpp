				   ...
				   bool allow_transient)
{
	...
	*granularity = 1ULL << ST1_XLAT_TABLE_LVL_SHIFT(level);
	...
	for (unsigned i = 0U; i < 3U; i++) {
		mmap[i].base_va = start_va + (page_idxs[i] * (*granularity));
...
TEST(stage1_xlat, xlat_get_table_from_va_TC1)
{
	 ...
	 * TEST CASE 1:
	 *
	 * For each possible end lookup level, create a set of
	 * random mappings starting at lookup levels from 0 up to
	 * ST1_XLAT_TEST_MAX_LEVEL.
	 * Then test xlat_get_table_from_va() on valid VAs from the
	 * mmap ranges and verify that the xlat_tbl_info structure is
	 * populated accordingly to the VA used.
	 ***************************************************************/
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		for (end_lvl = 1U;
			...
			for (start_lvl = 0U;
				...
				(void)gen_mmap_array_by_level(&init_mmap[0U],
							      &page_idx[0U],
							      &expected_lvls[0U],
							      mmap_count,
							      end_lvl,
				...
				(void)xlat_ctx_cfg_init(&cfg, region,
							&init_mmap[0U],
							mmap_count, va_size);
				...
				unsigned long long test_va = ctx.cfg->base_va;
				test_va += init_mmap[1U].base_va;
				 ...
				 */
				(void)st1_xlat_test_helpers_table_walk(&ctx,
								test_va,
								&tte,
								&(tbl_val.table),
								&(tbl_val.level),
