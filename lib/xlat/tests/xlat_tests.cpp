	...
	*granularity = 1ULL << XLAT_TESTS_TABLE_LVL_SHIFT(last_lvl);
	...
	for (unsigned i = 0U; i < 3U; i++) {
		mmap[i].base_va = mmap_start_va + (tbl_idxs[i]
							* (*granularity));
		/*
		 * PA can be anyone (as long as there are not overlpas, for
				 ...
				 unsigned int *expected_levels)
{
	...
	for (unsigned int i = 0U; i < ctx->cfg->mmap_regions; i++) {
		...
		retval = xlat_test_helpers_gen_attrs(ctx, test_va, &attrs);
...
TEST(xlat_tests, xlat_get_tte_ptr_TC1)
{
	...
	unsigned int test_idx[3U] =
				{0ULL, 0ULL, XLAT_TESTS_TBL_ENTRIES - 1U};
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		 ...
		 * We use a minimum of three granules.
		 */
		granules =
			(unsigned int)test_helpers_get_rand_in_range(3, 100);

		/* Generate a random mmap region */
		init_mmap.attr = get_mmap_attrs();
		init_mmap.base_va = start_va;
		init_mmap.base_pa = start_va & XLAT_TESTS_PA_MASK;
		init_mmap.size = granules * XLAT_TESTS_GRANULARITY_SIZE;
		init_mmap.granularity = XLAT_TESTS_MAX_BLOCK_SIZE;
		...
		test_va = ctx.cfg->base_va + init_mmap.base_va +
			(test_granule * XLAT_TESTS_GRANULARITY_SIZE);
		retval = xlat_get_table_from_va(&tbl_info, &ctx, test_va);
