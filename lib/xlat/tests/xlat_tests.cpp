...
TEST(xlat_tests, xlat_get_llt_from_va_TC1)
{
	 ...
	 * context. This will help verifying that xlat_get_llt_from_va()
	 * is able to calculate the right base VA of a given table
	 * ('struct xlat_llt_info.base_va' field).
	 *
	 * Then verify that the call to xlat_get_llt_from_va() is able
	 * to return the right xlat_llt_info structure with the expected
	 * values.
	 ***************************************************************/
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		for (end_lvl = 1U;
			...
			for (start_lvl = 0U;
				...
				/*
				 * The base VA offset mapped by the returned
				 * tte will correspond to the base_va on
				 * init_mmap[0]
				 */
				tbl_val.llt_base_offset = init_mmap[0U].base_va;

				/* Final VA offset */
				tbl_val.llt_end_offset = tbl_val.llt_base_offset;
				tbl_val.llt_end_offset +=
					XLAT_TESTS_TABLE_LVL_BLOCK_SIZE(end_lvl - 1U) - 1ULL;

				/* Base VA */
				tbl_val.base_va = ctx.cfg->base_va;
...
TEST(xlat_tests, xlat_get_llt_from_va_TC2)
{
	 ...
	 * TEST CASE 2:
	 *
	 * Try calling xlat_get_llt_from_va() with a VAs ouside
...
ASSERT_TEST(xlat_tests, xlat_get_llt_from_va_TC3)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
TEST(xlat_tests, xlat_get_llt_from_va_TC7)
{
	struct xlat_ctx ctx;
	struct xlat_ctx_cfg cfg;
	struct xlat_ctx_tbls tbls;
...
TEST(xlat_tests, xlat_get_tte_ptr_TC1)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/* Get the xlat_llt_info stucture used to look for TTEs */
...
TEST(xlat_tests, xlat_get_tte_ptr_TC2)
{
	...
	for (unsigned int i = 0U; i < (unsigned int)VA_REGIONS; i++) {
		...
		/*
		 * Get the xlat_llt_info stucture used to look for TTEs so we
		...
		VERBOSE("\nBase VA mapped by TT at level %u: 0x%lx\n",
				tbl_info.level, tbl_info.llt_base_offset +
						tbl_info.base_va);
		 ...
		 */
		test_va = tbl_info.llt_base_offset + tbl_info.base_va;
		...
		CHECK_VERBOSE((tte_ptr == NULL),
			      ...
			      test_va, tbl_info.llt_base_offset +
			      	       tbl_info.base_va);
		 ...
		 */
		test_va = tbl_info.llt_base_offset + tbl_info.base_va;
		test_va += XLAT_TESTS_TABLE_LVL_BLOCK_SIZE(2U);
		test_va += test_helpers_get_rand_in_range(1,
					XLAT_TESTS_GRANULARITY_SIZE - 1);
