...
uint64_t xlat_test_helpers_get_oa_from_tte(uint64_t tte)
{
	...
	if (is_feat_lpa2_4k_present() == true) {
		tte_oa = BIT_MASK_ULL(TTE_OA_BIT_49_LPA2, OA_ADDR_SHIFT);
		tte_oa &= tte;
		tte_oa |= INPLACE(OA_BITS_50_51, EXTRACT(TTE_OA_BITS_50_51, tte));
	} else {
		tte_oa = BIT_MASK_ULL(TTE_OA_ADDR_MSB, OA_ADDR_SHIFT);
		tte_oa &= tte;
	}
