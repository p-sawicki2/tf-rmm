				 ...
				 unsigned int *index)
{
	...
	for (unsigned int i = cfg->base_level;
		...
		case 0U:
			...
			tte_oa = (EXTRACT(XLAT_TESTS_TTE_ADDR, ctte) <<
						XLAT_TESTS_TTE_ADDR_SHIFT);
...
int xlat_test_helpers_gen_attrs(uint64_t *attrs, uint64_t mmap_attrs)
{
	...
	case MT_DEVICE:
		...
		upper_attrs = UPPER_ATTRS(PXN);
		break;
	case MT_NON_CACHEABLE:
		lower_attrs = LOWER_ATTRS(ATTR_NON_CACHEABLE_INDEX);
		upper_attrs = 0ULL;
		break;
	case MT_MEMORY:
