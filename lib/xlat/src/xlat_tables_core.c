...
static inline uint64_t xlat_map_pa_to_tte(uint64_t addr_pa)
{
	...
	if (is_feat_lpa2_4k_present() == true) {
		desc &= ~OA_BITS_50_51_MASK;
		desc |= INPLACE(TTE_OA_BITS_50_51, EXTRACT(OA_BITS_50_51, addr_pa));
	}
...
uint64_t xlat_desc(uint64_t attr, uintptr_t addr_pa, int level)
{
	...
	desc = xlat_map_pa_to_tte(addr_pa);
