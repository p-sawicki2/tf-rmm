					...
					int level)
{
	...
	while (table_idx < table_entries) {
		...
		if (action == ACTION_WRITE_BLOCK_ENTRY) {
			...
			subtable = get_oa_from_desc(desc);
			/* Recurse to write into subtable */
			/* FIXME: This violates misra-c2012-17.2 */
			end_va = xlat_tables_map_region(ctx, mm, table_idx_va,
					       subtable, XLAT_TABLE_ENTRIES,
...
uint64_t xlat_desc(uint64_t attr, uintptr_t addr_pa, int level)
{
	...
	desc = (lpa2_enabled == false) ? addr_pa :
		((addr_pa & ~OA_MSB_MASK) |
			INPLACE(TTE_OA_MSB, EXTRACT(OA_MSB, addr_pa)));
	...
	if (lpa2_enabled == false) {
		...
		desc |= LOWER_ATTRS(INPLACE(LOWER_ATTR_SH, ISH));
	}
...
int xlat_init_tables_ctx(struct xlat_ctx *ctx)
{
	 ...
	 * For this initialization, consider all the tables of the same size
	 * regardless of the level at which they will be mapped.
	 */
