...
uint64_t xlat_desc(uint64_t attr, uintptr_t addr_pa, int level)
{
	...
	/* Make sure that the address is within range */
	if (addr_pa > ((1ULL << arch_feat_get_pa_width()) - 1ULL)) {
		ERROR("%s (%u): 0x%lx larger than maximum valid address\n",
			__func__, __LINE__, addr_pa);
		panic();
	}
