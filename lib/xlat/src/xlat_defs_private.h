...
/*
 * Shareability attributes in TCR_EL2 when FEAT_LPA2 is enabled.
 *
 * Only ISH is supported.
 */
#define ISH			(UL(0x3))
...
static inline uint64_t *get_oa_from_tte(uint64_t tte)
{
	uint64_t oa;

	if (is_feat_lpa2_4k_present() == true) {
		oa = BIT_MASK_ULL(TTE_OA_BIT_49_LPA2, OA_ADDR_SHIFT);
		oa &= tte;
		oa |= INPLACE(OA_BITS_50_51, EXTRACT(TTE_OA_BITS_50_51, tte));
	} else {
		oa = BIT_MASK_ULL(TTE_OA_ADDR_MSB, OA_ADDR_SHIFT);
		oa &= tte;
	}
