...
#define BLOCK_DESC		VALID_DESC /* Table levels [1, 2] */
...
#define TABLE_ADDR_MSB		(48U)
#define TABLE_ADDR_MSB_LPA2	(50U)
...
static inline uint64_t *get_oa_from_desc(uint64_t desc)
{
	...
	if (is_feat_lpa2_4k_present() == true) {
		oa = BIT_MASK_ULL(TABLE_ADDR_MSB_LPA2, TABLE_ADDR_SHIFT);
		oa &= desc;
		oa |= INPLACE(OA_MSB, EXTRACT(TTE_OA_MSB, desc));
	} else {
		oa = BIT_MASK_ULL(TABLE_ADDR_MSB, TABLE_ADDR_SHIFT);
		oa &= desc;
	}
