			      ...
			      size_t va_size)
{
	...
	if (XLAT_TABLES_CTX_TBL_VALID(ctx) || XLAT_TABLES_CTX_CFG_VALID(ctx)) {
		return -EALREADY;
	}
			   ...
			   unsigned int ntables)
{
	...
	/* The tables configuration structure must not be initialzied */
	if (tbls_ctx->initialized == true) {
		return -EINVAL;
	}
	 ...
	 */
	if (base_table == NULL || base_level_entries == 0U) {
		return -EINVAL;
	}
