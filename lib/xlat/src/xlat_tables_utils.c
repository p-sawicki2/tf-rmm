			 ...
			 const uintptr_t va)
{
	...
	llt->table = table;
	llt->level = level;
	llt->base_va = ctx->cfg->base_va;
	llt->llt_base_offset = tt_base_va - ctx->cfg->base_va;
	llt->llt_end_offset = XLAT_BLOCK_SIZE(level - 1U) - 1ULL +
							llt->llt_base_offset;
	llt->llt_end_offset =
		(llt->llt_end_offset < ctx->cfg->max_mapped_va_offset) ?
			llt->llt_end_offset : ctx->cfg->max_mapped_va_offset;
 ...
 * last level table pointed to by 'llt', this function will not check for
 * INVALID_DESC entries in the table or make any assumptions on how many
 * entries in the table are in use.
 */
			   ...
			   const uintptr_t va)
{
	...
	/*
	 * For the low level region, it is guaranteed that va >= llt->base_va
	 * ('base_va' is 0x0). For the high level region, though, there is the
	 * possibility that va < llt_base_va.
	 */
	assert(va >= llt->base_va);

	offset = va - llt->base_va;

	/*
	 * Verify that va is higher or equal than the base VA
	 * mapped by the current table.
	 */
	if (offset < llt->llt_base_offset) {
		return NULL;
	}
