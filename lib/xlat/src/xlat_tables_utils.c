				      ...
				      uintptr_t * const tt_base_va)
{
	...
	if (va_offset > ctx->cfg->max_mapped_va_offset) {
		return NULL;
	}
 ...
 * will return the information regardless the last level table that could be
 * walked, regardless of whether 'va' is mapped or not.
 *
