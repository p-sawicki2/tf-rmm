 ...
 * context.
 */
int xlat_unmap_memory_page(struct xlat_tte_info * const table,
			   const uintptr_t va)
{
	uint64_t *tte;
	...
	tte = xlat_get_pte_from_table(table, va);
 ...
 * context.
 */
int xlat_map_memory_page_with_attrs(const struct xlat_tte_info * const table,
				    const uintptr_t va,
				    const uintptr_t pa,
				    const uint64_t attrs)
{
	...
	tte_ptr = xlat_get_pte_from_table(table, va);
 ...
 */
int xlat_get_table_from_va(struct xlat_tte_info * const retval,
			   const struct xlat_ctx * const ctx,
			   const uintptr_t va, size_t va_size)
{
	uintptr_t page_va;
	uint64_t *table;
	unsigned int level;
	...
	if (level == ctx_cfg->base_level) {
		retval->entries = GET_NUM_BASE_LEVEL_ENTRIES(va_size);
	} else {
 ...
 */
uint64_t *xlat_get_pte_from_table(const struct xlat_tte_info * const entry,
				  const uintptr_t va)
{
	unsigned int index;
	uint64_t *table;
