	...
	while (table_idx < XLAT_GET_TABLE_ENTRIES(level)) {
		...
		if ((is_invalid == true) || (is_transient_inv == true)) {
			...
			if (((desc & DESC_MASK) == TABLE_DESC) &&
				...
				addr_inner = get_oa_from_desc(desc);
				...
				VERBOSE("%sVA:0x%lx PA:0x%lx size:0x%zx ",
				       ...
				       (uint64_t)get_oa_from_desc(desc),
				       level_size);
				      ...
				      uintptr_t * const tt_base_va)
{
	...
	for (int level = start_level;
		...
		/* Get the next table */
		ret_table = get_oa_from_desc(desc);
	}
			   ...
			   const uintptr_t va)
{
	...
	assert(llt->level >= ((is_feat_lpa2_4k_present() == true) ?
						XLAT_TABLE_LEVEL_MIN :
						XLAT_TABLE_LEVEL_MIN + 1));
