	...
	while (table_idx < XLAT_GET_TABLE_ENTRIES(level)) {
		...
		if ((is_invalid == true) || (is_transient_inv == true)) {
			...
			if (((desc & DESC_MASK) == TABLE_DESC) &&
				...
				addr_inner =
					(uint64_t *)(void *)get_oa_from_tte(desc);
				      ...
				      uintptr_t * const tt_base_va)
{
	...
	for (int level = start_level;
		...
		/* Get the next table */
		ret_table = (uint64_t *)(void *)get_oa_from_tte(desc);
	}
...
uint64_t get_oa_from_tte(uint64_t tte)
{
	uint64_t oa;

	if (is_feat_lpa2_4k_present() == true) {
		oa = tte & BIT_MASK_ULL(TTE_OA_BIT_49_LPA2, OA_ADDR_SHIFT);
		oa |= INPLACE(OA_BITS_50_51, EXTRACT(TTE_OA_BITS_50_51, tte));
	} else {
		oa = tte & BIT_MASK_ULL(TTE_OA_ADDR_MSB, OA_ADDR_SHIFT);
	}
	return oa;
}
