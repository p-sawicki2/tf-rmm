...
/*
 * RsiIoCoherent
 * Represents whether the output address of an IO mapping is within the
 * system coherent memory space.
 * Width: 1 bit
 */
#define RSI_IO_MAPPING_NON_COHERENT		U(0)
#define RSI_IO_MAPPING_COHERENT			U(1)
...
struct rsi_device_info {
	...
	SET_MEMBER_RSI(unsigned long cert_id, 0, 0x8);
	/* RsiHashAlgorithm: Algorithm used to generate device digests */
	SET_MEMBER_RSI(unsigned char hash_algo, 0x8, 0x40);
