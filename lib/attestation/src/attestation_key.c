...
int attest_setup_platform_token(void)
{
	...
	size_t token_hunk_len, remaining_len = 0;
	...
	do {
		...
		/*
		 * Retry if the EL3 has not managed to return the token. It could be
		 * that HES queue is full and RMM needs to retry again.
		 */
		/* coverity[misra_c_2012_rule_9_1_violation:SUPPRESS] */
		if ((token_hunk_len == 0UL) && (offset == 0UL)) {
			continue;
		}
