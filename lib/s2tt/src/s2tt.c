...
static unsigned long addr_level_mask(unsigned long addr, long level, bool lpa2)
{
 ...
 * - table
 */
static bool s2tte_has_pa(struct s2tt_context *s2_ctx,
...
/*
 * Creates an empty TTE containing only the PA.
 * This function does not make any assumption not checks on the given PA.
 */
static unsigned long create_empty_tte(unsigned long pa, bool lpa2)
{
...
static unsigned long s2_addr_to_idx(unsigned long addr, long level)
{
	...
	unsigned int s2tte_stride = (level == S2TT_MIN_STARTING_LEVEL_LPA2) ?
...
static unsigned long table_get_entry(struct s2tt_context *s2_ctx,
...
#define table_entry_to_phys(tte, lpa2)			\
				addr_level_mask(tte, S2TT_PAGE_LEVEL, lpa2)

static struct granule *find_next_level_idx(struct s2tt_context *s2_ctx,
					   struct granule *g_tbl,
					   unsigned long idx)
{
	assert(s2_ctx != NULL);
	...
	return addr_to_granule(table_entry_to_phys(entry, s2_ctx->use_lpa2));
}

static struct granule *find_lock_next_level(struct s2tt_context *s2_ctx,
					    struct granule *g_tbl,
 ...
 *	 already locked.
 */
void s2tt_walk_lock_unlock(struct s2tt_context *s2_ctx,
			   unsigned long map_addr,
			   long level,
			   struct s2tt_walk *wi)
{
 ...
 */
unsigned long s2tte_create_unassigned_empty(struct s2tt_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
unsigned long s2tte_create_unassigned_ram(struct s2tt_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
unsigned long s2tte_create_unassigned_destroyed(struct s2tt_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
unsigned long s2tte_create_unassigned_ns(struct s2tt_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
static unsigned long s2tte_create_assigned(struct s2tt_context *s2_ctx,
					   ...
					   unsigned long s2tte_ripas)
{
	...
	unsigned long tte = create_empty_tte(pa, s2_ctx->use_lpa2);
 ...
 */
unsigned long s2tte_create_assigned_destroyed(struct s2tt_context *s2_ctx,
					      unsigned long pa, long level)
{
 ...
 */
unsigned long s2tte_create_assigned_empty(struct s2tt_context *s2_ctx,
					  unsigned long pa, long level)
{
	return s2tte_create_assigned(s2_ctx, pa, level,
 ...
 */
unsigned long s2tte_create_assigned_ram(struct s2tt_context *s2_ctx,
					unsigned long pa, long level)
{
	return s2tte_create_assigned(s2_ctx, pa, level,
 ...
 */
unsigned long s2tte_create_assigned_unchanged(struct s2tt_context *s2_ctx,
					      unsigned long s2tte,
					      unsigned long pa,
					      long level)
{
 ...
 */
unsigned long s2tte_create_assigned_ns(struct s2tt_context *s2_ctx,
				       unsigned long s2tte, long level)
{
	/*
	 * When FEAT_LPA2 is enabled, the two MSB of the OA are mapped
	 * to the Shareability bits, so the mask to use is effectively
	 * the same as when FEAT_LPA2 is disabled.
	 */
 ...
 */
bool host_ns_s2tte_is_valid(struct s2tt_context *s2_ctx,
			    unsigned long s2tte, long level)
{
	...
	lpa2 = s2_ctx->use_lpa2;

	mask = addr_level_mask(~0UL, level, lpa2);
	if (lpa2 == true) {
		mask |= S2TTE_NS_ATTR_LPA2_MASK | MASK(S2TTE_MSB);
		mask &= ~MASK(S2TTE_OA_MSB);
	} else {
 ...
 */
unsigned long host_ns_s2tte(struct s2tt_context *s2_ctx,
			    unsigned long s2tte, long level)
{
	assert(s2_ctx != NULL);

	unsigned long mask = addr_level_mask(~0UL, level, s2_ctx->use_lpa2);

	mask |= (s2_ctx->use_lpa2 == true) ? S2TTE_NS_ATTR_LPA2_MASK :
					     S2TTE_NS_ATTR_MASK;
 ...
 */
unsigned long s2tte_create_table(struct s2tt_context *s2_ctx,
				 unsigned long pa, long level)
{
	__unused unsigned int min_starting_level;

	(void)level;

	assert(s2_ctx != NULL);
	min_starting_level = (s2_ctx->use_lpa2 == true) ?
	...
	return (create_empty_tte(pa, s2_ctx->use_lpa2) | S2TTE_TABLE);
}
 ...
 * - assigned_destroyed
 */
bool s2tte_has_ripas(struct s2tt_context *s2_ctx,
		     unsigned long s2tte, long level)
{
 ...
 */
bool s2tte_is_unassigned(struct s2tt_context *s2_ctx, unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_empty(struct s2tt_context *s2_ctx,
			       unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_ram(struct s2tt_context *s2_ctx,
			     unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_ns(struct s2tt_context *s2_ctx,
			    unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_destroyed(struct s2tt_context *s2_ctx,
				   unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_assigned_destroyed(struct s2tt_context *s2_ctx,
				 unsigned long s2tte, long level)
{
	(void)level;
	(void)s2_ctx;
 ...
 */
bool s2tte_is_assigned_empty(struct s2tt_context *s2_ctx,
			     unsigned long s2tte, long level)
{
	(void)level;
	(void)s2_ctx;
...
static bool s2tte_check(struct s2tt_context *s2_ctx, unsigned long s2tte,
 ...
 */
bool s2tte_is_assigned_ram(struct s2tt_context *s2_ctx,
			   unsigned long s2tte, long level)
{
 ...
 */
bool s2tte_is_assigned_ns(struct s2tt_context *s2_ctx,
			  unsigned long s2tte, long level)
{
 ...
 */
bool s2tte_is_table(struct s2tt_context *s2_ctx, unsigned long s2tte,
		    long level)
{
	(void)s2_ctx;
 ...
 */
enum ripas s2tte_get_ripas(struct s2tt_context *s2_ctx, unsigned long s2tte)
{
 ...
 */
void s2tt_init_unassigned_empty(struct s2tt_context *s2_ctx,
				unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_unassigned_ram(struct s2tt_context *s2_ctx,
			      unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_unassigned_ns(struct s2tt_context *s2_ctx,
			     unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_unassigned_destroyed(struct s2tt_context *s2_ctx,
				    unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_destroyed(struct s2tt_context *s2_ctx,
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_empty(struct s2tt_context *s2_ctx,
			      unsigned long *s2tt, unsigned long pa,
			      long level)
{
	assert(level >= S2TT_MIN_BLOCK_LEVEL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_ram(struct s2tt_context *s2_ctx,
			    unsigned long *s2tt, unsigned long pa,
			    long level)
{
	assert(level >= S2TT_MIN_BLOCK_LEVEL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_ns(struct s2tt_context *s2_ctx,
			   ...
			   unsigned long pa, long level)
{
	...
	const unsigned long map_size = s2tte_map_size(level);
	unsigned long ns_attr_host_mask = (s2_ctx->use_lpa2 == true) ?
	...
	for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
		...
		s2tt[i] = s2tte_create_assigned_ns(s2_ctx,
				s2tte | create_empty_tte(pa, s2_ctx->use_lpa2),
				level);
		pa += map_size;
	}
 ...
 */
static bool s2tte_is_live(struct s2tt_context *s2_ctx,
			  unsigned long s2tte, long level)
{
...
unsigned long s2tte_pa(struct s2tt_context *s2_ctx, unsigned long s2tte,
		       long level)
{
	bool lpa2;
	unsigned long pa;
	...
	if (s2tte_is_table(s2_ctx, s2tte, level)) {
		...
		pa = addr_level_mask(s2tte, level, lpa2);
	}

	if (lpa2 == true) {
		pa &= ~(MASK(S2TTE_MSB) | MASK(S2TTE_OA_MSB));
		pa |= INPLACE(S2TTE_OA_MSB, EXTRACT(S2TTE_MSB, s2tte));
	}

	return pa;
}

bool s2tte_is_addr_lvl_aligned(struct s2tt_context *s2_ctx,
			      unsigned long addr, long level)
{
	assert(s2_ctx != NULL);

	__unused unsigned long min_starting_level = (s2_ctx->use_lpa2 == true) ?
		S2TT_MIN_STARTING_LEVEL_LPA2 : S2TT_MIN_STARTING_LEVEL;
	int levels = (int)(S2TT_PAGE_LEVEL - level);
...
typedef bool (*s2tte_type_checker)(struct s2tt_context *s2_ctx,
				   unsigned long s2tte);

static bool table_is_uniform_block(struct s2tt_context *s2_ctx,
				   unsigned long *table,
 ...
 */
bool s2tt_is_unassigned_empty_block(struct s2tt_context *s2_ctx,
				    unsigned long *table)
{
	assert(table != NULL);
 ...
 */
bool s2tt_is_unassigned_ram_block(struct s2tt_context *s2_ctx,
				  unsigned long *table)
{
	assert(table != NULL);
 ...
 */
bool s2tt_is_unassigned_ns_block(struct s2tt_context *s2_ctx,
				 unsigned long *table)
{
	assert(table != NULL);
 ...
 */
bool s2tt_is_unassigned_destroyed_block(struct s2tt_context *s2_ctx,
					unsigned long *table)
{
	assert(table != NULL);
...
typedef bool (*s2tte_type_level_checker)(struct s2tt_context *s2_ctx,
					 unsigned long s2tte, long level);

static bool table_maps_block(struct s2tt_context *s2_ctx,
			     unsigned long *table,
			     long level,
			     ...
			     bool check_ns_attrs)
{
	...
	ns_attr_host_mask = (s2_ctx->use_lpa2 == true) ?
		S2TTE_NS_ATTR_LPA2_MASK : S2TTE_NS_ATTR_MASK;
 ...
 */
bool s2tt_maps_assigned_empty_block(struct s2tt_context *s2_ctx,
 ...
 */
bool s2tt_maps_assigned_ram_block(struct s2tt_context *s2_ctx,
				  unsigned long *table, long level)
{
	return table_maps_block(s2_ctx, table, level,
 ...
 */
bool s2tt_maps_assigned_ns_block(struct s2tt_context *s2_ctx,
				 unsigned long *table, long level)
{
	return table_maps_block(s2_ctx, table, level,
 ...
 * refer to a contiguous block of granules aligned to @level - 1.
 */
bool s2tt_maps_assigned_destroyed_block(struct s2tt_context *s2_ctx,
					unsigned long *table, long level)
{
	return table_maps_block(s2_ctx, table, level,
 ...
 */
unsigned long s2tt_skip_non_live_entries(struct s2tt_context *s2_ctx,
					 unsigned long addr,
					 unsigned long *table,
