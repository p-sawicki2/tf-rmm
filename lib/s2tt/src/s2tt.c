					   ...
					   unsigned long idx)
{
	...
	return addr_to_granule(addr_level_mask(entry, S2TT_PAGE_LEVEL));
}
					   ...
					   unsigned long ripas)
{
	...
	assert(level <= S2TT_PAGE_LEVEL);
	assert(ripas <= S2TTE_INVALID_RIPAS_DESTROYED);
	assert(s2tt_is_addr_lvl_aligned(pa, level));
...
bool s2tte_has_ripas(unsigned long s2tte, long level)
{
	...
	if (s2tte_is_unassigned(s2tte) == true) {
		return (EXTRACT(S2TTE_INVALID_RIPAS, s2tte) != 3UL);
	}
	 ...
	 * DESTROYED or EMPTY.
	 */
	if ((s2tte & S2TT_DESC_TYPE_MASK) == S2TTE_Lx_INVALID) {
...
bool s2tt_is_addr_lvl_aligned(unsigned long addr, long level)
{
			     ...
			     bool check_ns_attrs)
{
	...
	if (!s2tt_is_addr_lvl_aligned(base_pa, level - 1L)) {
		return false;
	}
