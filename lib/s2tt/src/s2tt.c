...
static unsigned long s2tte_to_pa(unsigned long s2tte, long level, bool lpa2)
{
	...
	if (lpa2 == true) {
		pa &= ~(MASK(LPA2_OA_51_50) | MASK(LPA2_S2TTE_51_50));
 ...
 * This function expects 'pa' to be aligned.
 */
			      ...
			      unsigned long addr, long level)
{
	...
	int levels = (int)(S2TT_PAGE_LEVEL - level);
	unsigned int lsb = (levels * S2TTE_STRIDE) + GRANULE_SHIFT;
	unsigned int s2tte_oa_bits = (s2_ctx->enable_lpa2 == true) ?
