...
static bool host_ns_s2tte_is_valid_lpa2(unsigned long s2tte, long level)
{
	...
	mask = addr_level_mask(~0UL, level) | S2TTE_HOST_NS_ATTR_LPA2_MASK;
...
static unsigned long s2_addr_to_idx(unsigned long addr, long level)
{
	...
	int s2tte_stride = (level == RTT_MIN_STARTING_LEVEL_LPA2) ?
...
void s2tt_init_unassigned_empty(unsigned long *s2tt)
{
	...
	unsigned long s2tte = s2tte_create_unassigned_empty();
...
void s2tt_init_unassigned_destroyed(unsigned long *s2tt)
{
	...
	unsigned long s2tte = s2tte_create_unassigned_destroyed();
