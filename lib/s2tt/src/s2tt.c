 ...
 */
void s2tt_invalidate_page(const struct realm_s2_context *s2_ctx,
			  unsigned long addr)
{
 ...
 */
void s2tt_invalidate_block(const struct realm_s2_context *s2_ctx,
			   unsigned long addr)
{
	stage2_tlbi_ipa(s2_ctx, addr, GRANULE_SIZE);
}
...
static unsigned long table_get_entry(struct realm_s2_context *s2_ctx,
				     ...
				     unsigned long idx)
{
	...
	entry = s2tte_read(s2_ctx, &table[idx]);
	buffer_unmap(table);
...
static struct granule *find_next_level_idx(struct realm_s2_context *s2_ctx,
					   struct granule *g_tbl,
					   unsigned long idx)
{
...
static struct granule *find_lock_next_level(struct realm_s2_context *s2_ctx,
					    struct granule *g_tbl,
 ...
 *	 already locked.
 */
void s2tt_walk_lock_unlock(struct realm_s2_context *s2_ctx,
			   unsigned long map_addr,
			   long level,
			   struct s2tt_walk *wi)
{
 ...
 */
unsigned long s2tte_create_unassigned_empty(struct realm_s2_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
unsigned long s2tte_create_unassigned_ram(struct realm_s2_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
unsigned long s2tte_create_unassigned_destroyed(struct realm_s2_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 */
unsigned long s2tte_create_unassigned_ns(struct realm_s2_context *s2_ctx)
{
	(void)s2_ctx;
 ...
 * RIPAS=@ripas, at level @level.
 *
 * This function does not make any check on @ripas
 */
static unsigned long s2tte_create_assigned(struct realm_s2_context *s2_ctx,
 ...
 */
unsigned long s2tte_create_assigned_destroyed(struct realm_s2_context *s2_ctx,
					      unsigned long pa, long level)
{
 ...
 */
unsigned long s2tte_create_assigned_empty(struct realm_s2_context *s2_ctx,
					  unsigned long pa, long level)
{
	return s2tte_create_assigned(s2_ctx, pa, level,
 ...
 */
unsigned long s2tte_create_assigned_ram(struct realm_s2_context *s2_ctx,
					unsigned long pa, long level)
{
	return s2tte_create_assigned(s2_ctx, pa, level,
 ...
 */
unsigned long s2tte_create_assigned_unchanged(struct realm_s2_context *s2_ctx,
					      unsigned long s2tte,
					      unsigned long pa,
					      long level)
{
 ...
 * - Shareability
 */
unsigned long s2tte_create_assigned_ns(struct realm_s2_context *s2_ctx,
				       unsigned long s2tte, long level)
{
	...
	assert(level >= S2TT_MIN_BLOCK_LEVEL);
	assert(level <= S2TT_PAGE_LEVEL);
	assert((s2tte & S2TTE_NS_ATTR_RMM_MASK) == 0UL);
 ...
 */
bool host_ns_s2tte_is_valid(struct realm_s2_context *s2_ctx,
			    unsigned long s2tte, long level)
{
 ...
 */
unsigned long host_ns_s2tte(struct realm_s2_context *s2_ctx,
			    unsigned long s2tte, long level)
{
	unsigned long mask = addr_level_mask(~0UL, level) |
						S2TTE_NS_ATTR_HOST_MASK;
 ...
 */
unsigned long s2tte_create_table(struct realm_s2_context *s2_ctx,
				 unsigned long pa, long level)
{
	(void)level;
	(void)s2_ctx;
 ...
 * - assigned_destroyed
 */
bool s2tte_has_ripas(struct realm_s2_context *s2_ctx,
		     unsigned long s2tte, long level)
{
 ...
 */
bool s2tte_is_unassigned(struct realm_s2_context *s2_ctx, unsigned long s2tte)
{
 ...
 */
bool s2tte_is_unassigned_empty(struct realm_s2_context *s2_ctx,
			       unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_ram(struct realm_s2_context *s2_ctx,
			     unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_ns(struct realm_s2_context *s2_ctx,
			    unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_unassigned_destroyed(struct realm_s2_context *s2_ctx,
				   unsigned long s2tte)
{
	(void)s2_ctx;
 ...
 */
bool s2tte_is_assigned_destroyed(struct realm_s2_context *s2_ctx,
				 unsigned long s2tte, long level)
{
	(void)level;
	(void)s2_ctx;
 ...
 */
bool s2tte_is_assigned_empty(struct realm_s2_context *s2_ctx,
			     unsigned long s2tte, long level)
{
	(void)level;
	(void)s2_ctx;
...
static bool s2tte_check(struct realm_s2_context *s2_ctx, unsigned long s2tte,
 ...
 */
bool s2tte_is_assigned_ram(struct realm_s2_context *s2_ctx,
			   unsigned long s2tte, long level)
{
 ...
 */
bool s2tte_is_assigned_ns(struct realm_s2_context *s2_ctx,
			  unsigned long s2tte, long level)
{
 ...
 */
bool s2tte_is_table(struct realm_s2_context *s2_ctx, unsigned long s2tte,
		    long level)
{
	(void)s2_ctx;
 ...
 */
enum ripas s2tte_get_ripas(struct realm_s2_context *s2_ctx, unsigned long s2tte)
{
 ...
 */
void s2tt_init_unassigned_empty(struct realm_s2_context *s2_ctx,
				unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_unassigned_ram(struct realm_s2_context *s2_ctx,
			      unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_unassigned_ns(struct realm_s2_context *s2_ctx,
			     unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_unassigned_destroyed(struct realm_s2_context *s2_ctx,
				    unsigned long *s2tt)
{
	assert(s2tt != NULL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_destroyed(struct realm_s2_context *s2_ctx,
				  ...
				  long level)
{
	...
	const unsigned long map_size = s2tte_map_size(s2_ctx, level);
...
unsigned long s2tte_map_size(struct realm_s2_context *s2_ctx, long level)
{
	assert(level <= S2TT_PAGE_LEVEL);
	assert(level >= S2TT_MIN_STARTING_LEVEL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_empty(struct realm_s2_context *s2_ctx,
			      unsigned long *s2tt, unsigned long pa,
			      long level)
{
	assert(level >= S2TT_MIN_BLOCK_LEVEL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_ram(struct realm_s2_context *s2_ctx,
			    unsigned long *s2tt, unsigned long pa,
			    long level)
{
	assert(level >= S2TT_MIN_BLOCK_LEVEL);
 ...
 * hence, don't use s2tte_write for access.
 */
void s2tt_init_assigned_ns(struct realm_s2_context *s2_ctx,
 ...
 * - table
 */
static bool s2tte_has_pa(struct realm_s2_context *s2_ctx,
			 unsigned long s2tte, long level)
{
	unsigned long desc_type = s2tte & S2TT_DESC_TYPE_MASK;
 ...
 */
static bool s2tte_is_live(struct realm_s2_context *s2_ctx,
			  unsigned long s2tte, long level)
{
...
unsigned long s2tte_pa(struct realm_s2_context *s2_ctx, unsigned long s2tte,
		       long level)
{
	assert(level <= S2TT_PAGE_LEVEL);
	assert(level >= S2TT_MIN_STARTING_LEVEL);
...
bool s2tte_is_addr_lvl_aligned(struct realm_s2_context *s2_ctx,
...
typedef bool (*s2tte_type_checker)(struct realm_s2_context *s2_ctx,
				   unsigned long s2tte);

static bool table_is_uniform_block(struct realm_s2_context *s2_ctx,
				   unsigned long *table,
				   s2tte_type_checker s2tte_is_x)
{
	for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
		unsigned long s2tte = s2tte_read(s2_ctx, &table[i]);
 ...
 */
bool s2tt_is_unassigned_empty_block(struct realm_s2_context *s2_ctx,
				    unsigned long *table)
{
	assert(table != NULL);
 ...
 */
bool s2tt_is_unassigned_ram_block(struct realm_s2_context *s2_ctx,
				  unsigned long *table)
{
	assert(table != NULL);
 ...
 */
bool s2tt_is_unassigned_ns_block(struct realm_s2_context *s2_ctx,
				 unsigned long *table)
{
	assert(table != NULL);
 ...
 */
bool s2tt_is_unassigned_destroyed_block(struct realm_s2_context *s2_ctx,
					unsigned long *table)
{
	assert(table != NULL);
...
typedef bool (*s2tte_type_level_checker)(struct realm_s2_context *s2_ctx,
					 unsigned long s2tte, long level);

static bool table_maps_block(struct realm_s2_context *s2_ctx,
			     unsigned long *table,
			     long level,
			     ...
			     bool check_ns_attrs)
{
	...
	unsigned long map_size = s2tte_map_size(s2_ctx, level);
	unsigned long s2tte = s2tte_read(s2_ctx, &table[0]);
	unsigned int i;
	...
	for (i = 1U; i < S2TTES_PER_S2TT; i++) {
		...
		s2tte = s2tte_read(s2_ctx, &table[i]);
 ...
 */
bool s2tt_maps_assigned_empty_block(struct realm_s2_context *s2_ctx,
 ...
 */
bool s2tt_maps_assigned_ram_block(struct realm_s2_context *s2_ctx,
				  unsigned long *table, long level)
{
	return table_maps_block(s2_ctx, table, level,
 ...
 */
bool s2tt_maps_assigned_ns_block(struct realm_s2_context *s2_ctx,
				 unsigned long *table, long level)
{
	return table_maps_block(s2_ctx, table, level,
 ...
 * refer to a contiguous block of granules aligned to @level - 1.
 */
bool s2tt_maps_assigned_destroyed_block(struct realm_s2_context *s2_ctx,
					unsigned long *table, long level)
{
	return table_maps_block(s2_ctx, table, level,
 ...
 */
unsigned long s2tt_skip_non_live_entries(struct realm_s2_context *s2_ctx,
					 unsigned long addr,
					 unsigned long *table,
					 const struct s2tt_walk *wi)
{
	 ...
	 * return @addr.
	 */
	if (s2tte_is_live(s2_ctx, s2tte_read(s2_ctx, &table[index]), level)) {
		return addr;
	}
	 ...
	 */
	map_size = s2tte_map_size(s2_ctx, level);
	addr &= ~(map_size - 1UL);
