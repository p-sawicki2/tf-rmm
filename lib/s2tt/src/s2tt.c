...
/*
 * Set of variabldafs containing architectural values that depend of whether
...
static unsigned long (*host_ns_s2tte_cb)(unsigned long s2tte, long level);
static unsigned long (*s2tte_pa_cb)(unsigned long s2tte, long level);
...
/*
 * The functions below take a NS S2TTE and return the set of attributes
 * set by the host, including the PA.
 */
static unsigned long host_ns_s2tte_non_lpa2(unsigned long s2tte, long level)
{
	unsigned long mask;

	assert(level >= RTT_MIN_BLOCK_LEVEL);

	mask = addr_level_mask(~0UL, level) | s2tte_host_ns_attr_mask;
	return (s2tte & mask);
}

static unsigned long host_ns_s2tte_lpa2(unsigned long s2tte, long level)
{
	unsigned long mask;
	...
	mask = addr_level_mask(~0UL, level) | s2tte_host_ns_attr_mask;
	mask &= ~(MASK(S2TTE_OA_MSB) | MASK(S2TTE_MSB));
	mask |= MASK(S2TTE_MSB);
...
{
	unsigned long tte = pa & ~(MASK(S2TTE_OA_MSB) | MASK(S2TTE_MSB));
...
static bool host_ns_s2tte_is_valid_lpa2(unsigned long s2tte, long level)
{
	...
	mask = addr_level_mask(~0UL, level) | S2TTE_NS_ATTR_LPA2_MASK;
...
static bool host_ns_s2tte_is_valid_non_lpa2(unsigned long s2tte, long level)
{
	...
	mask = addr_level_mask(~0UL, level) | S2TTE_NS_ATTR_MASK;
...
/*
 * The following two functins return the physical address mapped to a
 * given S2TTE
 */
unsigned long s2tte_pa_non_lpa2(unsigned long s2tte, long level)
{
	assert(level <= RTT_PAGE_LEVEL);
	assert(level >= rtt_min_starting_level);
	assert(s2tte_has_pa(s2tte, level));
...
unsigned long s2tte_pa_lpa2(unsigned long s2tte, long level)
{
	unsigned long pa;
	...
	if (s2tte_is_table(s2tte, level)) {
		pa = addr_level_mask(s2tte, RTT_PAGE_LEVEL);
	} else {
		pa = addr_level_mask(s2tte, level);
	}
...
static unsigned long s2_addr_to_idx(unsigned long addr, long level)
{
	...
	int s2tte_stride = (level == RTT_STARTING_LEVEL_LPA2) ?
					   ...
					   unsigned long idx)
{
	...
	return addr_to_granule(s2tte_pa(entry, RTT_PAGE_LEVEL));
}
				    ...
				    unsigned long ripas)
{
	...
	return (create_empty_tte(pa) | S2TTE_INVALID_HIPAS_ASSIGNED | ripas);
}
...
{
	return host_ns_s2tte_cb(s2tte, level);
}
...
bool s2tte_has_ripas(unsigned long s2tte, long level)
{
	...
	if (((s2tte & S2TTE_NS) != 0UL) ||
		(s2tte_is_table(s2tte, level) == true)) {
		return false;
	}

	/* Verify if RIPAS is of a valid value for an unassigned S2TTE */
	/* TODO: Should we just add an assert() instead? */
	if (s2tte_is_unassigned(s2tte) == true) {
		return ((s2tte & S2TTE_INVALID_RIPAS_MASK) !=
						S2TTE_INVALID_RIPAS_INVALID);
	}

	/*
	 * For an invalid assigned descriptor, RIPAS can only be
	 * DESTROYED or EMPTY.
	 */
	/* TODO: Should we just add an assert() instead? */
	...
	/* The S2TTE is a protected assigned one with RIPAS == RAM */
	return true;
}
...
enum ripas s2tte_get_ripas(unsigned long s2tte)
{
	 ...
	 */
	assert(((s2tte & DESC_TYPE_MASK) == S2TTE_INVALID) ||
...
void s2tt_init_unassigned_empty(unsigned long *s2tt)
{
	...
	unsigned long s2tte =
		s2tte_create_unassigned(S2TTE_INVALID_RIPAS_EMPTY);
...
void s2tt_init_unassigned_ram(unsigned long *s2tt)
{
	...
	unsigned long s2tte = s2tte_create_unassigned(S2TTE_INVALID_RIPAS_RAM);
...
void s2tt_init_unassigned_ns(unsigned long *s2tt)
{
	...
	unsigned long s2tte = s2tte_create_unassigned(S2TTE_UNASSIGNED_NS);
				  ...
				  long level)
{
	...
	for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
		s2tt[i] = s2tte_create_assigned(pa, level,
						S2TTE_INVALID_RIPAS_DESTROYED);
		pa += map_size;
	}
	dsb(ish);
...
{
	return s2tte_pa_cb(s2tte, level);
}
