...
static unsigned long  s2tte_lvl_mask(unsigned long level, bool lpa2)
{
	assert(level <= S2TT_PAGE_LEVEL);

	int levels = (int)(S2TT_PAGE_LEVEL - level);
	unsigned int lsb = (levels * S2TTE_STRIDE) + GRANULE_SHIFT;
	unsigned long mask = BIT_MASK_ULL((S2TTE_OA_BITS - 1U), lsb);
					   ...
					   unsigned long s2tte_ripas)
{
	...
	unsigned long tte = pa_to_s2tte(pa, s2_ctx->use_lpa2);
			    ...
			    unsigned long s2tte, long level)
{
	...
	unsigned long mask = s2tte_lvl_mask(level, s2_ctx->use_lpa2);
				 ...
				 unsigned long pa, long level)
{
	...
	return (pa_to_s2tte(pa, s2_ctx->use_lpa2) | S2TTE_TABLE);
}
			   ...
			   unsigned long pa, long level)
{
	...
	for (unsigned int i = 0U; i < S2TTES_PER_S2TT; i++) {
		...
		s2tt[i] = s2tte_create_assigned_ns(s2_ctx,
				s2tte | pa_to_s2tte(pa, s2_ctx->use_lpa2),
				level);
		pa += map_size;
	}
