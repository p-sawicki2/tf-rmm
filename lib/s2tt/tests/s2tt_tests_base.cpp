...
void s2tte_has_ripas_tc2(void)
{
	...
	for (long level = s2tt_test_helpers_min_block_lvl();
		 ...
		 */
		tte = s2tte_create_assigned_empty(
				(const struct s2tt_context *)&s2tt_ctx,
				pa, level);
		tte &= ~S2TTE_INVALID_RIPAS_MASK;
...
void s2tt_assigned_empty_block_tc6(void)
{
	/***************************************************************
	 * TEST CASE 6:
	 *
	 * Invoke s2tt_assigned_empty_block() with a level below
	 * the minimum allowed.
	 ***************************************************************/
...
void s2tt_walk_lock_unlock_tc8(void)
{
	...
	/* The call should cause an assertion failure */
	test_helpers_expect_assert_fail(true);
	s2tt_walk_lock_unlock(g_tables[sl + 1U + 1U], sl + 1L, arch_feat_get_pa_width(),
			     pa, sl - 1U, &wi);
	test_helpers_fail_if_no_assert_failed();
#endif
}
