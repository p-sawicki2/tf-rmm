...
unsigned long s2tt_test_helpers_gen_ipa(long level, bool aligned)
{
	...
	do {
		...
		ipa = test_helpers_get_rand_in_range((1UL << lsb), ULONG_MAX);
		ipa &= ((1UL << shift) - 1UL);
	} while (ipa == 0UL);

	return aligned ? (ipa & s2tt_test_helpers_lvl_mask(level)) : ipa;
}
