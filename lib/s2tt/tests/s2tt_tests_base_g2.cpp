			   ...
			   struct granule *str_granule_base)
{
	...
	for (unsigned int i = 0U; i < n_granules; i++) {
		...
		s2tt_test_granule_set_state(granules[i], GRANULE_STATE_RTT);
		s2tt_test_granule_set_lock(granules[i], false);
	};

	/* Root granule must be locked. */
	next_granule = sl + 1L;
	s2tt_test_granule_set_lock(granules[next_granule], true);
