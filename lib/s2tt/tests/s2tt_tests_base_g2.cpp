...
void s2tt_invalidate_page_tc1(void)
{
	...
	struct s2tt_context s2tt_ctx = { 0UL };
	unsigned long ipa = s2tt_test_helpers_gen_pa(S2TT_TEST_HELPERS_MAX_LVL, true);
...
/*
 * Helper to implement a set of negative tests for s2tte_is_unassigned_x_block()
 * APIS:
 *
 ...
 ***************************************************************/
static void s2tt_is_x_block(unsigned int ripas)
{
	...
	/*
	 * pickup a random type of unassigned S2TTE to test with other than
	 * the RIPAS passed.
	 */
	do {
	...
	s2tt[tte_idx] = s2tt_test_create_unassigned(
				(const struct s2tt_context *)&s2tt_ctx,
				ripas_arr[idx]);
	CHECK_FALSE(s2tt_is_unassigned_x_block_cbs[ripas](
					(const struct s2tt_context *)&s2tt_ctx,
					&s2tt[0U]));
...
void s2tt_is_unassigned_ns_block_tc2(void)
{
	...
	s2tt_is_x_block(S2TTE_INVALID_NS_INDEX);
}
 ...
 *	       responsibility to ensure that the index at the minimum level
 *	       of table walk does not exceed S2TTE_MAX_CONCAT_TABLES.
 *	- tt_walk_idx: An array of indexes to the tables int tt_walk[] used
 *		       to walk the tables.
 ...
 *		       to tt_walk[] entries.
 *	- val_tt_granule: The spected sruct granule of the S2TT at the end of
 *			  test function.
 ...
 * the starting one (sl), which is sl+1. The `ipa` argumet is such that the
...
void s2tt_walk_lock_unlock_tc1(void)
{
	 ...
	 *	- Generate a random mapping starting at the minimum
	 ...
	 * but which walk index at SL + 1 is within
...
void s2tt_walk_lock_unlock_tc12(void)
{
	...
	/*
	 * We need a starting level at which we can always have addresses
	 * mapped at entries with index > S2TTE_MAX_CONCAT_TABLES.
	 */
	long sl = s2tt_test_helpers_min_table_lvl() + 1L;

	/*
	 * Generate a random address that spans across the whole IPA size
	 * but which walk table index at SL is larger or equal than
	 * S2TTE_MAX_CONCAT_TABLES so we can test concatenated tables.
	 *
	 * The address doesn't need to have any alignment.
	 */
	do {
