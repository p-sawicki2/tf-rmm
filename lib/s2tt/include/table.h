...
#define S2TTE_UNASSIGNED_NS		(S2TTE_INVALID_UNPROTECTED | \
					 S2TTE_NS)
/*
 * Descriptor types
 */
...
/*
 * Creates an unassigned s2tte with a given mask. This function does not make
 * any check on the 'mask' argument.
 */
static inline unsigned long s2tte_create_unassigned(unsigned long mask)
{
	return (S2TTE_INVALID_HIPAS_UNASSIGNED | mask);
}
