...
int q_init_check(struct circular_q *q)
{
	...
	if (q->read != 0 || q->length != 0 || q->capacity == 0 ||
	    q->element_size == 0 || q->buffer == NULL) {
		return -EINVAL;
	}

	if ((q->capacity & (q->capacity - 1)) != 0) {
		return -EINVAL;
	}
...
int q_push(struct circular_q *q, void *data, unsigned long size)
{
	...
	if (data == NULL || size == 0 || size > q->element_size) {
		return -EINVAL;
	}
	...
	index = (q->read + q->length) & (q->capacity - 1);
	buffer = (char *)q->buffer;
	memset(&buffer[index * q->element_size], 0, q->element_size);
	memcpy(&buffer[index * q->element_size], data, size);
	q->length++;
...
int q_pop(struct circular_q *q, void *data, unsigned long size)
{
	...
	buffer = (char *)q->buffer;
	memcpy(data, &buffer[q->read * q->element_size], q->element_size);
	memset(&buffer[q->read * q->element_size], 0, q->element_size);
	q->read = (q->read + 1) & (q->capacity - 1);
	q->length--;
...
void *q_iter_get_next(struct circular_q_iter *iter)
{
	...
	iter->index = (iter->index + 1) & (iter->q->capacity - 1);
	iter->remaining--;
