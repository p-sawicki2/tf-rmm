...
/* REFCOUNT_MAX can't exceed the value defined by refcount field width */
COMPILER_ASSERT(REFCOUNT_MAX <= ((1U << GRN_REFCOUNT_WIDTH) - 1U));
...
#define REFCOUNT(g)	\
	(SCA_READ16(&(g)->descriptor) &	(unsigned short)MASK(GRN_REFCOUNT))
...
/*
 * Return refcount value using atomic read with release semantics.
 *
...
static inline unsigned short granule_refcount_read_acquire(struct granule *g)
{
	...
	return SCA_READ16_ACQUIRE(&g->descriptor) &
...
static inline void granule_set_state(struct granule *g, unsigned char state)
{
	...
	val = (g->descriptor & (unsigned short)MASK(GRN_STATE));
...
{
	assert((g != NULL) && LOCKED(g));
	atomic_add_16(&g->descriptor, val);
}
...
static inline void granule_refcount_dec(struct granule *g, unsigned short val)
{
	...
	assert((g != NULL) && LOCKED(g));
	old_refcount = atomic_load_add_16(&g->descriptor,
					(uint16_t)(-val)) & MASK(GRN_REFCOUNT);
