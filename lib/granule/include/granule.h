...
/* Maxim value defined by the 'refcount' field width in granule descriptor */
							...
							unsigned char state)
{
	...
	case GRANULE_STATE_RD:
		 ...
		 * object. Can be any non-negative number less than
		 * REFCOUNT_MAX value.
		 */
...
static inline void granule_refcount_inc(struct granule *g, unsigned short val)
{
	...
	assert((g != NULL) && LOCKED(g));
	old_refcount = atomic_load_add_16(&g->descriptor, val);
