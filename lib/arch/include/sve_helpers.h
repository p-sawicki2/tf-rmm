 ...
 * Z register.
 */
#define SVE_Z_REG_SIZE		U(16)
#define SVE_P_REG_SIZE		(SVE_Z_REG_SIZE / 8)
#define SVE_FFR_REG_SIZE	(SVE_Z_REG_SIZE / 8)

/* Number of Z, P, FFR registers */
#define SVE_VQ_ARCH_MAX		(1 << ZCR_EL2_SVE_VL_WIDTH)
#define SVE_Z_REG_NUM		U(32)
#define SVE_P_REG_NUM		U(16)
#define SVE_FFR_REG_NUM		U(1)

#define SVE_Z_REGS_SIZE(vq)	((vq) * (SVE_Z_REG_SIZE * SVE_Z_REG_NUM))
#define SVE_P_REGS_SIZE(vq)	((vq) * (SVE_P_REG_SIZE * SVE_P_REG_NUM))
#define SVE_FFR_REGS_SIZE(vq)	((vq) * (SVE_FFR_REG_SIZE * SVE_FFR_REG_NUM))
...
#define SVE_STATE_OFFSET_FPCR	(SVE_STATE_OFFSET_FPSR + sizeof(unsigned long))

/*
 * Size of SVE Z, Predicate (P), First Fault predicate Register (FFR) registers
 * in bytes for vector length 128 bits (1 vq). P and FFR registers are 1/8 of
 * Z register.
 */
struct sve_state {
	__uint128_t z[SVE_VQ_ARCH_MAX][SVE_Z_REG_NUM];
	uint16_t p[SVE_VQ_ARCH_MAX][SVE_P_REG_NUM];
	uint16_t ffr[SVE_VQ_ARCH_MAX][SVE_FFR_REG_NUM];
	unsigned long zcr_el12;
	/*
