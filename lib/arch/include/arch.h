...
#define SCTLR_ELx_RES1		((UL(1) << 22) /* TODO: ARMv8.5-CSEH, otherwise RES1 */ | \
				 (UL(1) << 11) /* TODO: ARMv8.5-CSEH, otherwise RES1 */)

#define SCTLR_ELx_M		(UL(1) << 0)
#define SCTLR_ELx_C		(UL(1) << 2)
#define SCTLR_ELx_SA		(UL(1) << 3)
#define SCTLR_ELx_SA0		(UL(1) << 4)
#define SCTLR_ELx_CP15BEN	(UL(1) << 5)
#define SCTLR_ELx_nAA		(UL(1) << 6)
#define SCTLR_ELx_SED		(UL(1) << 8)
#define SCTLR_ELx_EOS		(UL(1) << 11)
#define SCTLR_ELx_I		(UL(1) << 12)
#define SCTLR_ELx_DZE		(UL(1) << 14)
#define SCTLR_ELx_UCT		(UL(1) << 15)
#define SCTLR_ELx_NTWI		(UL(1) << 16)
#define SCTLR_ELx_NTWE		(UL(1) << 18)
#define SCTLR_ELx_WXN		(UL(1) << 19)
#define SCTLR_ELx_TSCXT		(UL(1) << 20)
#define SCTLR_ELx_EIS		(UL(1) << 22)
#define SCTLR_ELx_SPAN		(UL(1) << 23)
#define SCTLR_ELx_EE		(UL(1) << 25)
#define SCTLR_ELx_UCI		(UL(1) << 26)
#define SCTLR_ELx_NTLSMD	(UL(1) << 28)
#define SCTLR_ELx_LSMAOE	(UL(1) << 29)
#define SCTLR_ELx_EnIA		(UL(1) << 31)

#define SCTLR_EL2_INIT		(/* SCTLR_EL2_M = 0 (MMU disabled) */  \
				/* SCTLR_EL2_A = 0 (No alignment checks) */  \
				 SCTLR_ELx_C   /* Data accesses are cacheable
						* as per translation tables */ | \
				 SCTLR_ELx_SA  /* SP aligned at EL2 */ | \
				 SCTLR_ELx_SA0  /* SP Alignment check enable for EL0 */ \
				...
				/* SCTLR_EL2_ITD = 0 (A32 Only) */ | \
				 SCTLR_ELx_SED /* A32 Only, RES1 for non-A32 systems */ \
				/* SCTLR_EL2_EOS TODO: ARMv8.5-CSEH, otherwise RES1 */ | \
				 SCTLR_ELx_I	 /* I$ is ON for EL2 and EL0 */ | \
				 SCTLR_ELx_DZE   /* Do not trap DC ZVA */ | \
				 SCTLR_ELx_UCT   /* Allow EL0 access to CTR_EL0 */ | \
				 SCTLR_ELx_NTWI  /* Don't trap WFI from EL0 to EL2 */ | \
				 SCTLR_ELx_NTWE  /* Don't trap WFE from EL0 to EL2 */ | \
				 SCTLR_ELx_WXN   /* W implies XN */ | \
				 SCTLR_ELx_TSCXT /* Trap EL0 accesss to SCXTNUM_EL0 */ | \
				 ...
				 * exceptions to EL2)) */ \
				 SCTLR_ELx_UCI /* Allow cache maintenance
						* instructions at EL0 */ | \
				 SCTLR_ELx_NTLSMD /* A32/T32 only */ | \
				 SCTLR_ELx_LSMAOE /* A32/T32 only */)

#define SCTLR_EL2_RUNTIME	(SCTLR_EL2_INIT | \
				 SCTLR_ELx_M   /* MMU enabled */)
