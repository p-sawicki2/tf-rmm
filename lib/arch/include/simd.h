...
enum simd_sflags {
	/* Set when context is initialized */
	SIMD_SFLAG_INIT_DONE = 1 << 0U,
	/*
	 * Set when context is saved in memory, cleared when context is restored
 ...
 * SIMD_OWNER_EL1: Context belongs to Realm
 */
typedef enum {
	SIMD_OWNER_NWD = 1U,
	SIMD_OWNER_EL1 = 2U,
...
/*
 * This structure holds the SIMD related EL2 registers that are not saved by EL3
 * firmware. As RMM will overwrite these registers, RMM will save and restore
 * NS world state during context switch
 */
...
struct simd_context {
	/* Owner of this context, can be SIMD_OWNER_NWD or SIMD_OWNER_EL1 */
	simd_owner_t owner;

	/* Bitwise OR of type flags */
	enum simd_tflags tflags;

	/* Bitwise OR of status flags */
	enum simd_sflags sflags;
	 ...
	 * SIMD_OWNER_EL1: RMM intializes based on Realm configuration
	 *
	 ...
	 * RMM will save and restore NS world EL2 registers on behalf of EL3
	 * since EL3 is not saving it (only for SIMD_OWNER_NWD).
	 */
	struct simd_el2_regs ns_el2_regs;

	/*
	 * SIMD related lower EL registers. Saved and restored across context
	 * switch.
	 */
	struct simd_common_sysregs {
...
/* Returns the SIMD config discovered during the init time */
int simd_get_system_config(struct simd_config *simd_cfg);

/* Initialize the SIMD context of NS world or Realm */
int simd_context_init(simd_owner_t owner, struct simd_context *simd,
		      struct simd_config *config);
