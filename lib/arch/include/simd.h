...
#define FPU_REGS_SIZE		(unsigned long)(FPU_VEC_REG_SIZE * FPU_VEC_REG_NUM)
...
#define SVE_P_REG_MIN_SIZE	U(SVE_Z_REG_MIN_SIZE / 8U)
#define SVE_FFR_REG_MIN_SIZE	U(SVE_Z_REG_MIN_SIZE / 8U)
...
#define SVE_VQ_ARCH_MAX		U((1UL << ZCR_EL2_LEN_WIDTH) - 1U)
...
struct sve_regs {
	...
	uint8_t z[(unsigned int)SVE_Z_REGS_SIZE(SVE_VQ_ARCH_MAX)];
	/* cppcheck-suppress misra-c2012-10.4 */
 ...
 */
COMPILER_ASSERT((unsigned int)(offsetof(struct fpu_regs, q)) == FPU_REGS_OFFSET_Q);
COMPILER_ASSERT((unsigned int)(offsetof(struct sve_regs, z)) == SVE_REGS_OFFSET_Z);
COMPILER_ASSERT((unsigned long)(offsetof(struct sve_regs, p)) == SVE_REGS_OFFSET_P);
COMPILER_ASSERT((unsigned long)(offsetof(struct sve_regs, ffr)) == SVE_REGS_OFFSET_FFR);
