...
void sve_save_regs_cb(struct sve_regs *regs, bool save_ffr)
{
	increment_times_called(SVE_SAVE_REGS);
}

void fpu_restore_regs_cb(struct fpu_regs *regs)
{
	increment_times_called(FPU_RESTORE_REGS);
}

void sve_restore_regs_cb(struct sve_regs *regs, bool restore_ffr)
{
...
TEST(simd, simd_context_switch_TC1)
{
	...
	unsigned int times_called_prev[SIMD_CB_IDS_MAX];
	int ret1, ret2;
	...
	(void)memcpy(times_called_prev, helpers_times_called,
		     sizeof(times_called_prev));
	...
	CHECK_TRUE(helpers_times_called[FPU_SAVE_REGS] -
		   times_called_prev[FPU_SAVE_REGS] == 1U);
	CHECK_TRUE(helpers_times_called[FPU_RESTORE_REGS] -
		   times_called_prev[FPU_RESTORE_REGS] == 1U);
	...
	CHECK_TRUE(helpers_times_called[FPU_SAVE_REGS] -
		   times_called_prev[FPU_SAVE_REGS] == 2U);
	CHECK_TRUE(helpers_times_called[FPU_RESTORE_REGS] -
		   times_called_prev[FPU_RESTORE_REGS] == 2U);
}
...
TEST(simd, simd_context_switch_TC2)
{
	...
	CHECK_TRUE(helpers_times_called[FPU_SAVE_REGS] -
		   times_called_prev[FPU_SAVE_REGS] == 1);
	CHECK_TRUE(helpers_times_called[SVE_RESTORE_REGS] -
		   times_called_prev[SVE_RESTORE_REGS] == 1);
	...
	CHECK_TRUE(helpers_times_called[SVE_SAVE_REGS] -
		   times_called_prev[SVE_SAVE_REGS] == 1);
	CHECK_TRUE(helpers_times_called[FPU_RESTORE_REGS] -
		   times_called_prev[FPU_RESTORE_REGS] == 1);
}
