...
ASSERT_TEST(simd, simd_context_restore_TC3)
{
	 ...
	 * TEST CASE 3:
	 *
	 * Call simd_context_restore() when a save has not yet been
	 * performed (of any context). Expect an assertion to fail.
	 ******************************************************************/
...
ASSERT_TEST(simd, simd_context_restore_TC4)
{
	...
	/*
	 * Restore the context for the first time. We use simd_context_switch(),
	 * with the same context for ctx_save and ctx_restore to do this, as
	 * the context must be saved first before it can be restored.
	 */
	(void)simd_context_switch(&test_simd_ctx, &test_simd_ctx);
