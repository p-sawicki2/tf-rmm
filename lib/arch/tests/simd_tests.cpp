...
TEST(simd, simd_init_TC2)
{
	...
	simd_test_helpers_setup_sysregs_sve();
...
TEST(simd, simd_context_init_TC3)
{
	 ...
	 * TEST CASE 3:
	 *
	 * Call simd_context_init() with an invalid simd_cfg (SVE enabled
	 * in simd_cfg but not enabled in CPU SIMD config). Expect the
...
TEST(simd, simd_context_init_TC4)
{
	 ...
	 * TEST CASE 4:
	 *
	 * Call simd_context_init() with an invalid simd_cfg (SVE VQ
	 * larger than SVE VQ of CPU SIMD config). Expect the function to
	 * exit early with exit code -1.
	 ******************************************************************/

	sve_vq = (SVE_VQ_ARCH_MAX - 1) * 128;
	cb.sve_rdvl = sve_rdvl_cb;
	 ...
	 */
	test_simd_cfg.sve_en = true;
	test_simd_cfg.sve_vq = cpu_simd_cfg.sve_vq + 1;
...
TEST(simd, simd_context_init_TC5)
{
	 ...
	 * TEST CASE 5:
	 *
	 * Call simd_context_init() with an invalid simd_cfg (SME enabled
	 * in simd_cfg but not enabled in CPU SIMD config). Expect the
	 * function to exit early with exit code -1.
	 ******************************************************************/

	sve_vq = SVE_VQ_ARCH_MAX * 128;
	cb.sve_rdvl = sve_rdvl_cb;
