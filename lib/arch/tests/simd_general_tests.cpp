...
TEST(simd_general, simd_get_cpu_config_TC1)
{
	 ...
	 * TEST CASE 1:
	 *
	 * Call simd_get_cpu_config() with a non-NULL simd_cfg to get the SIMD
	 * config that was discovered during init. Check that the config fields
	 * are correct for FPU.
	 ******************************************************************/
...
{
	struct simd_config *simd_cfg = (struct simd_config *)NULL;
