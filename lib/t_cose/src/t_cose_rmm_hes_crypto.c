				  ...
				  struct q_useful_buf signature_buffer)
{
	...
	memcpy(hes_ctx_locked->state.c_buffer_for_tbs_hash, hash_to_sign.ptr,
	       hash_to_sign.len);
}
				...
				uintptr_t granule_addr)
{
	...
	memset(&hes_ctx->state, 0, sizeof(hes_ctx->state));
	...
	struct q_useful_buf signature_buffer, struct q_useful_buf_c *signature)
{
	...
	if (hes_crypto_context->state.is_sig_valid) {
		...
		memcpy(signature_buffer.ptr, hes_crypto_context->state.sig_buffer,
					 ...
					 size_t *sig_size)
{
	...
	*sig_size = (size_t)PSA_SIGN_OUTPUT_SIZE(key_type, (int)key_len_bits,
						 psa_alg_id);
