 ...
 */
__attribute__((target("branch-protection=none"))) void pauth_init_enable_el2(void)
{
	uint64_t low;
	uint64_t high;
	uint64_t sctlr_el2;
	uint64_t cpu_id = my_cpuid();

	arch_collect_entropy(&low);
	arch_collect_entropy(&high);

	/*
	 * Allow PAuth Instruction access
	 ...
	 */
	write_sctlr_el2(sctlr_el2 | SCTLR_ELx_EnIA_BIT);
	isb();
}

__attribute__((target("branch-protection=none"))) void pauth_restore_rmm_keys(void)
{
...
__attribute__((target("branch-protection=none"))) void
pauth_restore_realm_keys(struct pauth_state *pauth)
{
